...
import {buildRecoveryCenter,recoveryMessage,buildWinningLibrary,recommendWinningCases,buildAdoptionAndValue,parseCsv,filterDealsForView} from '../modules/commercial-acceleration.js';
...
test('Fase 24: recuperação reúne proposta silenciosa e perda recuperável',()=>{const center=buildRecoveryCenter({deals:[{id:'d1',status:'lost',recoveryPotential:'high',lossReason:'Prazo',client:'A',title:'Contrato',value:10000}],proposals:[{id:'p1',status:'viewed',client:'B',title:'Projeto',value:5000,lastViewedAt:'2026-08-20'}],activities:[],today:'2026-08-27'});assert.equal(center.items.length,2);assert.equal(center.recoverableValue,15000);assert.match(recoveryMessage(center.items.find(item=>item.proposal)),/acessou a proposta/)});
test('Fase 25: venda ganha vira caso reutilizável',()=>{const cases=buildWinningLibrary([{id:'d1',status:'won',client:'Empresa A',title:'Contrato',value:20000,pain:'Retrabalho',objection:'Preço',successCriteria:'Prazo'}],[{name:'Empresa A',segment:'Serviços'}]);assert.equal(cases[0].segment,'Serviços');assert.equal(cases[0].score,75);assert.equal(recommendWinningCases({pain:'retrabalho'},cases)[0].id,'d1')});
test('Fase 26: painel mede implantação e receita protegida',()=>{const value=buildAdoptionAndValue({company:{name:'Empresa'},clients:[{id:'c1'}],deals:[{id:'d1',status:'open',value:12000,next:'Ligar',nextDate:'2026-08-30'}],activities:[],proposals:[],conversations:[]});assert.ok(value.progress>=50);assert.equal(value.metrics.protectedRevenue,12000)});
test('Fase 27: CSV aceita ponto e vírgula e visões filtram o funil',()=>{const rows=parseCsv('Nome;E-mail\nEmpresa A;contato@a.com\nEmpresa B;b@b.com');assert.equal(rows.length,2);assert.equal(rows[0].email,'contato@a.com');const filtered=filterDealsForView([{id:'d1'},{id:'d2',next:'Ligar',nextDate:'2026-08-30'}],'without-next');assert.deepEqual(filtered.map(item=>item.id),['d1'])});
