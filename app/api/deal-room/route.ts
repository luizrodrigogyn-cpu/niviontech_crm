...
export async function POST(request:Request){
  ...
  const payload=safeRoomPayload(body.payload),code=String(body.accessCode||'').replace(/\D/g,'').slice(0,6),days=Math.max(1,Math.min(30,Number(body.expiresInDays)||7));
  ...
  return response({token,url:`/sala/?token=${token}`,expiresAt:expiresAt.toISOString()});
}
