...
function renderPipeline(search=''){
  ...
  $('#pipelineTotal').textContent=formatMoney(deals.reduce((sum,deal)=>sum+Number(deal.value),0));
  ...
  $('#kanbanBoard').innerHTML=pipelineStages.map(stage=>{
    ...
    return `<section class="kanban-column" data-stage="${stage.id}"><header class="column-header"><div><i class="stage-dot"></i><h3>${stage.label}</h3><span class="column-count">${stageDeals.length}</span></div><span class="column-value">${formatMoney(total)}</span></header><div>${stageDeals.length?stageDeals.map(dealCard).join(''):orbitEmptyState('Etapa livre',`O Orbit avisa quando uma oportunidade estiver pronta para ${stage.label.toLowerCase()}.`,'empty-column')}</div></section>`;
  }).join('');
  bindDragAndDrop();
}
...
function moveDeal(id,newStage,method){const deals=getDeals();const deal=deals.find(item=>item.id===id);if(!deal||deal.stage===newStage)return;const previous=deal.stage,from=pipelineStages.find(stage=>stage.id===previous)?.label,to=pipelineStages.find(stage=>stage.id===newStage)?.label;deal.stage=newStage;deal.movedAt=new Date().toISOString();addEntityHistory(deal,'Etapa alterada',`${from} → ${to} · ${method}`);saveDeals(deals);appState.lastMoveAction={dealId:id,from:previous,to:newStage};showUndo(`${deal.title}: ${from} → ${to}`);renderPipeline($('#dealSearch').value)}
...
function undoLastMove(){if(!appState.lastMoveAction)return;const deals=getDeals(),deal=deals.find(item=>item.id===appState.lastMoveAction.dealId);if(deal&&deal.stage===appState.lastMoveAction.to){const from=pipelineStages.find(stage=>stage.id===appState.lastMoveAction.to)?.label,to=pipelineStages.find(stage=>stage.id===appState.lastMoveAction.from)?.label;deal.stage=appState.lastMoveAction.from;addEntityHistory(deal,'Movimentação desfeita',`${from} → ${to}`);saveDeals(deals);renderPipeline($('#dealSearch').value)}hideUndo()}
...
function updateDashboardMetrics(){const deals=dealsVisibleToCurrentUser(getDeals()).filter(deal=>deal.status!=='lost'),clients=getClients(),open=deals.filter(deal=>deal.paymentStatus!=='received'),total=open.reduce((sum,deal)=>sum+Number(deal.value),0),closing=open.filter(deal=>deal.stage==='closing').reduce((sum,deal)=>sum+Number(deal.value),0);$('#todayOpenDeals').textContent=open.length;$('#todayPipelineValue').textContent=`${formatMoney(total)} no funil`;$('#todayClosingValue').textContent=formatMoney(closing);$('#todayClientsCount').textContent=clients.length}
...
function renderTemplates(){const active=funnelTemplates.find(item=>item.id===pipelineConfig.templateId);$('#currentTemplateName').textContent=active?.name||'Funil personalizado';$('#templateGrid').innerHTML=funnelTemplates.slice(1).map(template=>`<article class="template-card ${pipelineConfig.templateId===template.id?'active':''}"><span class="template-icon">${template.icon}</span><h3>${template.name}</h3><p>${template.description}</p><div class="template-stages">${template.stages.slice(0,4).map(([,label])=>`<span>${label}</span>`).join('')}${template.stages.length>4?`<span>+${template.stages.length-4}</span>`:''}</div><button data-apply-template="${template.id}">${pipelineConfig.templateId===template.id?'Modelo em uso':'Usar este modelo'}</button></article>`).join('');$('#activeStagesPreview').innerHTML=pipelineStages.map((stage,index)=>`<div class="stage-preview-item"><span>${stage.label}</span>${index<pipelineStages.length-1?'<i>→</i>':''}</div>`).join('');document.querySelectorAll('[data-apply-template]').forEach(button=>button.onclick=()=>applyFunnelTemplate(button.dataset.applyTemplate))}
function applyFunnelTemplate(templateId){const template=funnelTemplates.find(item=>item.id===templateId);if(!template||templateId===pipelineConfig.templateId)return;const message=`Aplicar o modelo “${template.name}”? As oportunidades existentes serão distribuídas proporcionalmente nas novas etapas e cada mudança ficará registrada no histórico.`;if(!confirm(message))return;const previousStages=[...pipelineStages],newStages=template.stages.map(([id,label])=>({id,label})),deals=getDeals();deals.forEach(deal=>{const oldIndex=Math.max(0,previousStages.findIndex(stage=>stage.id===deal.stage)),ratio=previousStages.length>1?oldIndex/(previousStages.length-1):0,newIndex=Math.min(newStages.length-1,Math.round(ratio*(newStages.length-1))),oldLabel=previousStages[oldIndex]?.label||deal.stage;deal.stage=newStages[newIndex].id;deal.movedAt=new Date().toISOString();addEntityHistory(deal,'Funil reconfigurado',`${oldLabel} → ${newStages[newIndex].label} · Modelo ${template.name}`)});saveDeals(deals);pipelineConfig={templateId,stages:newStages,updatedAt:new Date().toISOString()};pipelineStages=newStages;localStorage.setItem(PIPELINE_CONFIG_KEY,JSON.stringify(pipelineConfig));renderTemplates()}
function stageByMeaning(meaning){const terms={new:['novo','lead','conversa','contato'],qualified:['qualifica','entendimento','interesse','diagnóstico'],proposal:['proposta','orçamento','oferta'],closing:['fechamento','negociação','aprovado','fechado']}[meaning]||[];return pipelineStages.find(stage=>terms.some(term=>stage.label.toLowerCase().includes(term)))?.id||pipelineStages[Math.min({new:0,qualified:1,proposal:2,closing:pipelineStages.length-1}[meaning]||0,pipelineStages.length-1)].id}
...
$('#markLost').onclick=()=>{if(confirm('Confirmar esta oportunidade como perdida?'))updateActiveDeal({status:'lost',lostAt:new Date().toISOString()},'Venda perdida','Negociação marcada como perdida pelo usuário')};
...
function renderReports(){
  ...
  const forecast=open.reduce((total,deal)=>{
    const index=Math.max(0,pipelineStages.findIndex(stage=>stage.id===deal.stage));
    const probability=Math.min(.85,.15+(index/Math.max(1,pipelineStages.length-1))*.7);
...
function renderForecastTimeline(deals){
  ...
  open.forEach(deal=>{
    ...
    const stageIndex=Math.max(0,pipelineStages.findIndex(stage=>stage.id===deal.stage));
    const probability=Math.min(.9,.15+(stageIndex/Math.max(1,pipelineStages.length-1))*.75);
...
function refreshStageAccumulatedValues(){
  ...
  const deals=dealsVisibleToCurrentUser(getDeals()).filter(deal=>deal.status!=='lost');
  columns.slice(0,pipelineStages.length).forEach((column,index)=>{
    ...
    if(!stageId)return;
    const progress=Math.round(index/Math.max(1,pipelineStages.length-1)*100);
    column.classList.add('stage-semantic-progress');
    column.style.setProperty('--stage-progress-color','color-mix(in srgb, var(--navy) '+(100-progress)+'%, var(--status-success) '+progress+'%)');
...
function renderFunnelEditorRows(){
  ...
  list.innerHTML=funnelEditorDraft.map((stage,index)=>{
    const count=deals.filter(deal=>deal.stage===stage.id&&deal.status!=='lost').length;
    return'<div class="funnel-editor-row" data-editor-index="'+index+'"><span class="funnel-drag-handle">'+String(index+1).padStart(2,'0')+'</span><label><small>NOME DA ETAPA</small><input type="text" value="'+escapeHtml(stage.label)+'" maxlength="32" data-stage-label="'+index+'"></label><span class="funnel-stage-count">'+count+' '+(count===1?'card':'cards')+'</span><div class="funnel-row-actions"><button type="button" data-stage-up="'+index+'" '+(index===0?'disabled':'')+' aria-label="Mover etapa para esquerda">←</button><button type="button" data-stage-down="'+index+'" '+(index===funnelEditorDraft.length-1?'disabled':'')+' aria-label="Mover etapa para direita">→</button><button type="button" data-stage-remove="'+index+'" '+(count?'disabled title="Mova os cards antes de remover"':'')+' aria-label="Remover etapa">×</button></div></div>';
  }).join('');
...
function installFunnelCustomization(){
  ...
  setTimeout(()=>{
    try{const custom=JSON.parse(localStorage.getItem(CUSTOM_PIPELINE_KEY));if(Array.isArray(custom)&&custom.length>=2){pipelineStages.splice(0,pipelineStages.length,...custom);if(appState.currentUser&&typeof renderPipeline==='function')renderPipeline()}}catch(error){}
  },0);
}
...
function applyTodayMetricSemantics(){
  ...
  const closingStage=pipelineStages[Math.max(0,pipelineStages.length-1)]?.id;
