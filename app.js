...
function moveDeal(id,newStage,method){const deals=getDeals();const deal=deals.find(item=>item.id===id);if(!deal||deal.stage===newStage)return;const previous=deal.stage,from=pipelineStages.find(stage=>stage.id===previous)?.label,to=pipelineStages.find(stage=>stage.id===newStage)?.label,previousState={status:deal.status,paymentStatus:deal.paymentStatus,wonAt:deal.wonAt,lostAt:deal.lostAt};deal.stage=newStage;deal.movedAt=new Date().toISOString();if(newStage==='won'){applyWonDealRules(deal);deal.wonAt=deal.wonAt||new Date().toISOString()}else if(newStage==='after-sales'){deal.status='won';deal.wonAt=deal.wonAt||new Date().toISOString();deal.paymentStatus=deal.paymentStatus||'pending'}else if(newStage==='lost'){deal.status='lost';deal.lostAt=new Date().toISOString()}else if(deal.status==='won'||deal.status==='lost'){deal.status='open'}addEntityHistory(deal,'Etapa alterada',`${from} → ${to} · ${method}`);saveDeals(deals);appState.lastMoveAction={dealId:id,from:previous,to:newStage,...previousState};showUndo(`${deal.title}: ${from} → ${to}`);renderPipeline($('#dealSearch').value)}
