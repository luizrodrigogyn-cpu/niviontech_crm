...
function analyzeDealConversation(){const text=$('#analysisText').value.trim(),client=getClients().find(item=>item.id===$('#analysisClientSelect').value),deal=getDeals().find(item=>item.id===$('#analysisDealSelect').value),target=$('#analysisOutput');if(!client){$('#analysisClientSelect').focus();return}if(!text){$('#analysisText').focus();return}const signals=analyzeCommercialConversation(text,{client,deal,icp:getOrbitIcp()}),nextDate=deal?.nextDate||todayISO();pendingOrbitAnalysis={id:crypto.randomUUID(),clientId:client.id,dealId:deal?.id||'',text,summary:signals.summary,signals,risk:signals.risk,next:signals.next,nextDate,date:new Date().toISOString(),actor:appState.currentUser?.name||'Usuário'};target.innerHTML=`<div class="orbit-analysis-head"><span class="orbit-large">O</span><div><p class="overline">DIAGNÓSTICO ORBIT · REVISE ANTES DE SALVAR</p><h3>${escapeHtml(signals.reportTitle)}</h3><small>A conversa virou um plano comercial acionável.</small></div></div><div class="analysis-signals"><article><small>QUALIFICAÇÃO</small><strong>${signals.score}%</strong></article><article><small>SAÚDE</small><strong>${signals.risk}</strong></article><article><small>OBJEÇÕES</small><strong>${signals.objections.length}</strong></article><article><small>SINAIS DE COMPRA</small><strong>${signals.buying.length}</strong></article></div><label>Resumo executivo<textarea id="analysisSummaryEdit" rows="4">${escapeHtml(signals.summary)}</textarea></label><div class="orbit-intelligence-grid"><article><small>LACUNAS PARA AVANÇAR</small><p>${escapeHtml(signals.gaps.join(' · ')||'Os principais critérios foram identificados.')}</p></article><article><small>COMO RESPONDER À OBJEÇÃO</small><p>${escapeHtml(signals.objectionGuidance)}</p></article></div><div class="analysis-next-grid"><label>Próximo passo<input id="analysisNextEdit" value="${escapeHtml(signals.next)}"></label><label>Data da ação<input id="analysisNextDateEdit" type="date" value="${escapeHtml(nextDate)}"></label></div><label>Follow-up pronto para copiar<textarea id="analysisFollowUpEdit" rows="4">${escapeHtml(signals.followUp)}</textarea></label><div class="analysis-review"><p><b>Dores:</b> ${escapeHtml(signals.pains.join(', ')||'não identificadas')}</p><p><b>Objeções:</b> ${escapeHtml(signals.objections.join(', ')||'não identificadas')}</p><p><b>Valor citado:</b> ${escapeHtml(signals.money||'não identificado')}</p></div><button class="primary wide" type="button" id="confirmOrbitAnalysis">Confirmar, atualizar CRM e criar próxima ação</button>`;$('#confirmOrbitAnalysis').onclick=confirmOrbitAnalysis}
function confirmOrbitAnalysis(){if(!pendingOrbitAnalysis)return;pendingOrbitAnalysis.summary=$('#analysisSummaryEdit').value.trim();pendingOrbitAnalysis.next=$('#analysisNextEdit').value.trim();pendingOrbitAnalysis.nextDate=$('#analysisNextDateEdit').value;pendingOrbitAnalysis.followUp=$('#analysisFollowUpEdit').value.trim();const clients=getClients(),client=clients.find(item=>item.id===pendingOrbitAnalysis.clientId);if(!client)return;client.orbitMemory={summary:pendingOrbitAnalysis.summary,pains:pendingOrbitAnalysis.signals.pains,objections:pendingOrbitAnalysis.signals.objections,lastConversationAt:pendingOrbitAnalysis.date,lastNextStep:pendingOrbitAnalysis.next,risk:pendingOrbitAnalysis.risk,qualificationScore:pendingOrbitAnalysis.signals.score,gaps:pendingOrbitAnalysis.signals.gaps,followUp:pendingOrbitAnalysis.followUp};client.interactions=client.interactions||[];client.interactions.unshift({id:'interaction-'+Date.now(),title:'Diagnóstico comercial criado pelo Orbit IA',text:pendingOrbitAnalysis.summary,date:pendingOrbitAnalysis.date});addEntityHistory(client,'Memória atualizada pelo Orbit IA',pendingOrbitAnalysis.summary);saveClients(clients);if(pendingOrbitAnalysis.dealId){const deals=getDeals(),deal=deals.find(item=>item.id===pendingOrbitAnalysis.dealId);if(deal){deal.orbitMemory={summary:pendingOrbitAnalysis.summary,pains:pendingOrbitAnalysis.signals.pains,objections:pendingOrbitAnalysis.signals.objections,budget:pendingOrbitAnalysis.signals.money,urgency:pendingOrbitAnalysis.signals.date,risk:pendingOrbitAnalysis.risk,qualificationScore:pendingOrbitAnalysis.signals.score,gaps:pendingOrbitAnalysis.signals.gaps,followUp:pendingOrbitAnalysis.followUp,updatedAt:pendingOrbitAnalysis.date};deal.next=pendingOrbitAnalysis.next||deal.next;deal.nextDate=pendingOrbitAnalysis.nextDate||deal.nextDate;deal.pain=deal.pain||pendingOrbitAnalysis.signals.pains[0]||'';deal.objection=deal.objection||pendingOrbitAnalysis.signals.objections[0]||'';deal.budget=deal.budget||pendingOrbitAnalysis.signals.money||'';addEntityHistory(deal,'Diagnóstico comercial criado pelo Orbit IA',`${pendingOrbitAnalysis.summary} · Qualificação ${pendingOrbitAnalysis.signals.score}%`);saveDeals(deals)}}if(pendingOrbitAnalysis.next&&pendingOrbitAnalysis.nextDate){const activities=getActivities(),duplicate=activities.some(item=>!item.done&&item.client===client.name&&item.title===pendingOrbitAnalysis.next&&item.date===pendingOrbitAnalysis.nextDate);if(!duplicate){activities.push({id:'activity-'+Date.now(),title:pendingOrbitAnalysis.next,type:'Follow-up',client:client.name,date:pendingOrbitAnalysis.nextDate,time:'09:00',note:'Criada automaticamente após diagnóstico do Orbit IA',owner:appState.currentUser?.name||'',done:false});saveActivities(activities)}}const conversations=getOrbitConversations();conversations.unshift(pendingOrbitAnalysis);saveOrbitConversations(conversations);$('#analysisOutput').innerHTML='<div class="orbit-saved-state"><span>✓</span><h3>CRM atualizado</h3><p>O diagnóstico, a memória da negociação, o follow-up e a próxima atividade foram organizados. O vendedor já sabe o que fazer agora.</p><button class="secondary" type="button" data-view="today">Ver próxima ação</button></div>';$('#analysisOutput [data-view]').onclick=()=>showView('today');$('#analysisText').value='';pendingOrbitAnalysis=null}
