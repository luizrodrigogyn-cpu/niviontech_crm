...
function syncGuidedNavigation(view){
  const groups={deals:['pipeline','activities','proposals','closingPlans','cadences','channels','receipts'],clients:['clients','success'],orbit:['orbitCoach','organize','playbooks'],management:['reports','growthOs','ranking','team','templates','settings']};
...
function renderDealJourney(deal,{proposals,activities}){
  ...
  const steps=[
    ...
    {key:'money',label:'Resultado',state:received?`${formatMoney(deal.receivedAmount||deal.value)} recebido`:deal.status==='won'?'Venda ganha · receber':'Em negociação',done:received,action:'money'}
  ];
