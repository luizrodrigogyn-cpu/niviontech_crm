...
function openDealDrawer(id){
  ...
  $('#dealConnectedContext').innerHTML=`<article class="connected-context-card"><small>CLIENTE</small><strong>${escapeHtml(client?.name||deal.client)}</strong><p>${escapeHtml(client?.mainContact||client?.phone||'Contato principal ainda não definido')}</p></article><article class="connected-context-card"><small>ATIVIDADES PENDENTES</small><strong>${activities.length}</strong><p>${escapeHtml(activities[0]?.title||'Nenhuma ação pendente')}</p></article><article class="connected-context-card"><small>PROPOSTAS</small><strong>${proposals.length}</strong><p>${escapeHtml(proposals[0]?.title||'Nenhuma proposta vinculada')}</p></article>`;
...
function changeDealStage(stage){const deal=getDeals().find(item=>item.id===appState.activeDealId);if(!deal||deal.stage===stage)return;if(!confirmStageAdvance(deal,stage))return;const from=pipelineStages.find(item=>item.id===deal.stage)?.label,to=pipelineStages.find(item=>item.id===stage)?.label;updateActiveDeal({stage,movedAt:new Date().toISOString()},'Etapa alterada',`${from} → ${to}`)}
function saveDealDetailChanges(){const deals=getDeals(),deal=deals.find(item=>item.id===appState.activeDealId);if(!deal)return;const previousOwner=deal.owner,previousRole=deal.ownerRole||'Colaborador comercial',newOwner=$('#drawerDealOwner').value.trim(),newRole=$('#drawerDealRole').value,reason=$('#drawerTransferReason').value.trim(),ownerChanged=newOwner!==previousOwner||newRole!==previousRole;deal.value=Number($('#drawerDealValue').value);deal.owner=newOwner;deal.ownerRole=newRole;deal.next=$('#drawerDealNext').value.trim();deal.nextDate=$('#drawerDealNextDate').value;deal.pain=$('#drawerDealPain').value.trim();deal.decisionMaker=$('#drawerDealDecisionMaker').value.trim();deal.budget=$('#drawerDealBudget').value.trim();deal.objection=$('#drawerDealObjection').value.trim();deal.urgency=$('#drawerDealUrgency').value;deal.competitor=$('#drawerDealCompetitor').value.trim();deal.successCriteria=$('#drawerDealSuccessCriteria').value.trim();deal.updatedAt=new Date().toISOString();const validation=validateNegotiation(deal);if(!validation.valid){alert(validation.message);return}if(ownerChanged){const stageLabel=pipelineStages.find(stage=>stage.id===deal.stage)?.label||deal.stage,handoff=createHandoffSummary(deal,clientForDeal(deal),{fromOwner:previousOwner,toOwner:newOwner,fromRole:previousRole,toRole:newRole,reason,stageLabel});deal.handoffSummary=handoff;addEntityHistory(deal,'Responsável transferido',`${previousOwner} (${previousRole}) → ${newOwner} (${newRole})${reason?` · Motivo: ${reason}`:''}`);addEntityHistory(deal,'Resumo de passagem criado',handoff.text,{handoffSummary:handoff.text})}else addEntityHistory(deal,'Plano da negociação atualizado','Próximo passo e critérios comerciais revisados');saveDeals(deals);openDealDrawer(deal.id);if(appState.currentView==='pipeline')renderPipeline($('#dealSearch').value);if(appState.currentView==='today')renderTodayActivities()}
...
function openClientDrawer(id){
  ...
  $('#drawerClientMainContact').value=client.mainContact||'';$('#drawerClientDecisionMaker').value=client.decisionMaker||'';$('#drawerClientAccountOwner').value=client.accountOwner||'';$('#drawerClientIcpFit').value=client.icpFit||'unknown';$('#drawerClientGoal').value=client.goal||'';$('#drawerClientStrategicNotes').value=client.strategicNotes||'';
...
function renderOrbitCoach(){
  ...
  renderCoachContext();renderCoachHistory()
}
...
function prepareSelectedDeal(){const deal=getDeals().find(item=>item.id===$('#prepareDealSelect').value),target=$('#prepareOutput');if(!deal){$('#prepareDealSelect').focus();return}const client=clientForDeal(deal)||{},icp=getOrbitIcp(),memory=deal.orbitMemory||{},missing=[];if(!memory.pains?.length)missing.push('Qual impacto do problema atual?');if(!memory.decisionMakers?.length)missing.push('Quem participa da decisão?');if(!memory.budget)missing.push('Existe uma faixa de investimento?');if(!memory.urgency)missing.push('Qual é o prazo ou urgência?');const objections=splitTerms(icp.objections).slice(0,3);target.innerHTML=`<div class="orbit-prep-head"><span class="orbit-large">O</span><div><p class="overline">PREPARAÇÃO ORBIT</p><h3>${escapeHtml(client.name||deal.client)}</h3><small>${escapeHtml(deal.title)} · ${formatMoney(deal.value)}</small></div></div><div class="orbit-prep-section"><strong>Objetivo recomendado</strong><p>Avançar a negociação em <b>${escapeHtml(deal.next||'um próximo passo com data e responsável')}</b>.</p></div><div class="orbit-prep-section"><strong>Perguntas que faltam</strong>${(missing.length?missing:['Confirmar se as informações registradas continuam válidas.']).map(item=>`<p>→ ${escapeHtml(item)}</p>`).join('')}</div><div class="orbit-prep-section"><strong>Objeções para antecipar</strong><p>${escapeHtml(objections.join(' · ')||'Nenhuma objeção configurada no ICP.')}</p></div><div class="orbit-prep-section"><strong>Contexto disponível</strong><p>${escapeHtml(memory.summary||client.orbitMemory?.summary||'Ainda não existe uma memória de conversa para esta oportunidade.')}</p></div>`}
function detectConversationSignals(text){const lower=text.toLocaleLowerCase('pt-BR'),find=(terms)=>terms.filter(term=>lower.includes(term)),money=text.match(/R\$\s?[\d.]+(?:,\d{2})?/i)?.[0]||'',date=text.match(/\b(?:\d{1,2}\/\d{1,2}(?:\/\d{2,4})?|segunda|terça|quarta|quinta|sexta)(?:-feira)?\b/i)?.[0]||'',objections=find(['caro','preço','orçamento','sem tempo','não tenho interesse','preciso pensar','falar com meu sócio','concorrente','já usamos']),pains=find(['planilha','desorganização','perda','atraso','retrabalho','falta de controle','sem visibilidade','dificuldade']),buying=find(['interesse','gostei','proposta','contrato','começar','fechar','avançar']),nextMatch=text.match(/(?:retornar|ligar|enviar|agendar|marcar|reunião|apresentar)[^.\n]{0,100}/i)?.[0]||'';return{money,date,objections,pains,buying,next:nextMatch}}
let pendingOrbitAnalysis=null;
function analyzeDealConversation(){const text=$('#analysisText').value.trim(),client=getClients().find(item=>item.id===$('#analysisClientSelect').value),deal=getDeals().find(item=>item.id===$('#analysisDealSelect').value),target=$('#analysisOutput');if(!client){$('#analysisClientSelect').focus();return}if(!text){$('#analysisText').focus();return}const signals=detectConversationSignals(text),sentences=text.split(/[.!?\n]+/).map(item=>item.trim()).filter(Boolean),summary=sentences.slice(0,3).join('. ').slice(0,520),risk=signals.objections.length?'Atenção':signals.buying.length?'Saudável':'Informações insuficientes',next=signals.next||deal?.next||'Definir próximo passo com data e responsável';pendingOrbitAnalysis={id:crypto.randomUUID(),clientId:client.id,dealId:deal?.id||'',text,summary,signals,risk,next,date:new Date().toISOString(),actor:appState.currentUser?.name||'Usuário'};target.innerHTML=`<div class="orbit-analysis-head"><span class="orbit-large">O</span><div><p class="overline">RASCUNHO · NADA SALVO</p><h3>Leitura da conversa</h3><small>Revise antes de alimentar o CRM.</small></div></div><label>Resumo<textarea id="analysisSummaryEdit" rows="5">${escapeHtml(summary)}</textarea></label><div class="analysis-signals"><article><small>SAÚDE</small><strong>${risk}</strong></article><article><small>VALOR CITADO</small><strong>${escapeHtml(signals.money||'Não identificado')}</strong></article><article><small>OBJEÇÕES</small><strong>${signals.objections.length}</strong></article><article><small>SINAIS DE COMPRA</small><strong>${signals.buying.length}</strong></article></div><label>Próximo passo<input id="analysisNextEdit" value="${escapeHtml(next)}"></label><div class="analysis-review"><p><b>Dores:</b> ${escapeHtml(signals.pains.join(', ')||'não identificadas')}</p><p><b>Objeções:</b> ${escapeHtml(signals.objections.join(', ')||'não identificadas')}</p></div><button class="primary wide" type="button" id="confirmOrbitAnalysis">Confirmar e alimentar o CRM</button>`;$('#confirmOrbitAnalysis').onclick=confirmOrbitAnalysis}
function confirmOrbitAnalysis(){if(!pendingOrbitAnalysis)return;pendingOrbitAnalysis.summary=$('#analysisSummaryEdit').value.trim();pendingOrbitAnalysis.next=$('#analysisNextEdit').value.trim();const clients=getClients(),client=clients.find(item=>item.id===pendingOrbitAnalysis.clientId);if(!client)return;client.orbitMemory={summary:pendingOrbitAnalysis.summary,pains:pendingOrbitAnalysis.signals.pains,objections:pendingOrbitAnalysis.signals.objections,lastConversationAt:pendingOrbitAnalysis.date,lastNextStep:pendingOrbitAnalysis.next,risk:pendingOrbitAnalysis.risk};client.interactions=client.interactions||[];client.interactions.unshift({id:'interaction-'+Date.now(),title:'Conversa analisada pelo Orbit IA',text:pendingOrbitAnalysis.summary,date:pendingOrbitAnalysis.date});addEntityHistory(client,'Memória atualizada pelo Orbit IA',pendingOrbitAnalysis.summary);saveClients(clients);if(pendingOrbitAnalysis.dealId){const deals=getDeals(),deal=deals.find(item=>item.id===pendingOrbitAnalysis.dealId);if(deal){deal.orbitMemory={summary:pendingOrbitAnalysis.summary,pains:pendingOrbitAnalysis.signals.pains,objections:pendingOrbitAnalysis.signals.objections,budget:pendingOrbitAnalysis.signals.money,urgency:pendingOrbitAnalysis.signals.date,risk:pendingOrbitAnalysis.risk,updatedAt:pendingOrbitAnalysis.date};deal.next=pendingOrbitAnalysis.next||deal.next;addEntityHistory(deal,'Conversa analisada pelo Orbit IA',pendingOrbitAnalysis.summary);saveDeals(deals)}}const conversations=getOrbitConversations();conversations.unshift(pendingOrbitAnalysis);saveOrbitConversations(conversations);$('#analysisOutput').innerHTML='<div class="orbit-saved-state"><span>✓</span><h3>Memória atualizada</h3><p>A análise foi vinculada ao cliente e à oportunidade. O histórico e o próximo passo também foram atualizados.</p><button class="secondary" type="button" data-view="clients">Ver cliente</button></div>';$('#analysisOutput [data-view]').onclick=()=>showView('clients');$('#analysisText').value='';pendingOrbitAnalysis=null}
...
$('#clientStrategyForm').onsubmit=event=>{event.preventDefault();const clients=getClients(),client=clients.find(item=>item.id===appState.activeClientId);if(!client)return;client.mainContact=$('#drawerClientMainContact').value.trim();client.decisionMaker=$('#drawerClientDecisionMaker').value.trim();client.accountOwner=$('#drawerClientAccountOwner').value.trim();client.icpFit=$('#drawerClientIcpFit').value;client.goal=$('#drawerClientGoal').value.trim();client.strategicNotes=$('#drawerClientStrategicNotes').value.trim();client.updatedAt=new Date().toISOString();addEntityHistory(client,'Mapa do relacionamento atualizado','Contato, decisão, aderência ao ICP e objetivo comercial revisados');saveClients(clients);openClientDrawer(client.id)};
