...
import {activitiesDomain,todayISO,formatDate,rankTodayActivities,shiftActivity,completeActivityWithNext} from './modules/activities.js';
import {proposalsDomain,proposalStatusLabel,calculateProposalTotals,markProposalViewed,acceptProposal,proposalFamily,createProposalRevision,compareProposalVersions,proposalFollowUp} from './modules/proposals.js';
...
import {buildRecoveryCenter,recoveryMessage,buildWinningLibrary,recommendWinningCases,buildAdoptionAndValue,parseCsv,defaultSavedViews,filterDealsForView} from './modules/commercial-acceleration.js';
...
const CATALOG_KEY='niviontech_catalog';
...
function showView(view){
  ...
  if(!pipeline)$('#orbitAttentionTrigger')?.classList.remove('pipeline-no-recommendation');
...
function renderPlaybookDealGuide(){
  ...
  target.innerHTML=`<section class="playbook-readiness ${evaluation.ready?'ready':''}"><div><small>PRONTIDÃO PARA AVANÇAR</small><strong>${evaluation.percent}%</strong></div><span><i style="width:${evaluation.percent}%"></i></span><p>${evaluation.ready?'Todos os critérios desta etapa estão presentes.':`Falta preencher: ${escapeHtml(formatMissingPlaybookCriteria(evaluation.missing))}.`}</p></section><section class="playbook-guide-block"><small>OBJETIVO DA ETAPA</small><h4>${escapeHtml(guide.goal)}</h4></section><section class="playbook-guide-block"><small>PERGUNTAS RECOMENDADAS</small><ol>${guide.questions.map(question=>`<li>${escapeHtml(question)}</li>`).join('')}</ol></section><section class="playbook-guide-block objections"><small>RESPOSTAS PARA OBJEÇÕES</small>${guide.objections.map((item,index)=>`<article><strong>${escapeHtml(item.trigger)}</strong><p>${escapeHtml(item.response)}</p><button type="button" data-copy-playbook="${index}">Copiar resposta</button></article>`).join('')}</section><button type="button" class="primary wide" id="applyPlaybookAction">Criar próxima ação: ${escapeHtml(guide.action)}</button>`;
...
function renderClosingPlans(){
  ...
  const activeValue=active.reduce((sum,item)=>sum+Number(openDeals.find(deal=>deal.id===item.plan.dealId)?.value||0),0),nextItems=active.map(item=>({plan:item.plan,next:item.progress.next})).filter(item=>item.next).sort((a,b)=>a.next.dueDate.localeCompare(b.next.dueDate)),nearest=nextItems[0];$('#closingCommandValue').textContent=formatMoney(activeValue);$('#closingCommandNext').textContent=nearest?.next?.title||'Nenhum';$('#closingCommandNextDate').textContent=nearest?`${nearest.plan.client} · ${formatDate(nearest.next.dueDate)}`:'Aguardando roteiro';$('#closingCommandHealth').textContent=overdue.length?'Atenção':'Organizado';$('#closingCommandHealthText').textContent=overdue.length?`${overdue.length} ${overdue.length===1?'plano com atraso':'planos com atraso'}`:'sem atrasos críticos';$('#closingCommandTitle').textContent=overdue.length?'Há compromissos que precisam voltar ao ritmo.':active.length?'O fechamento está sendo conduzido.':'Conduza o caminho até o sim.';
...
function renderRecoveryCenter(){const center=buildRecoveryCenter({deals:getDeals(),proposals:getProposals(),activities:getActivities(),today:todayISO()});$('#recoveryValue').textContent=formatMoney(center.recoverableValue);$('#recoveryWithoutAction').textContent=center.withoutAction;$('#recoveryViewed').textContent=center.viewedSilent;$('#recoveryLost').textContent=center.lostCandidates;$('#recoveryTotal').textContent=center.items.length;$('#recoveryQueue').innerHTML=center.items.length?center.items.map((item,index)=>`<article class="recovery-card"><span>${String(index+1).padStart(2,'0')}</span><div><small>${escapeHtml(item.kind)} · ${escapeHtml(item.reason)}</small><h3>${escapeHtml(item.client)} · ${escapeHtml(item.title)}</h3><p>${escapeHtml(recoveryMessage(item))}</p></div><strong>${formatMoney(item.value)}</strong><div class="recovery-actions"><button type="button" data-recovery-copy="${escapeHtml(item.id)}">Copiar mensagem</button><button type="button" class="primary" data-recovery-activate="${escapeHtml(item.id)}">${item.hasPending?'Abrir ação':'Criar recuperação'}</button></div></article>`).join(''):orbitEmptyState('Nenhuma receita esquecida','Propostas e negociações estão acompanhadas com próximos passos.');document.querySelectorAll('[data-recovery-copy]').forEach(button=>button.onclick=async()=>{const item=center.items.find(entry=>entry.id===button.dataset.recoveryCopy),message=recoveryMessage(item);try{await navigator.clipboard.writeText(message);button.textContent='Mensagem copiada ✓'}catch{alert(message)}});document.querySelectorAll('[data-recovery-activate]').forEach(button=>button.onclick=()=>activateRecovery(center.items.find(entry=>entry.id===button.dataset.recoveryActivate))) }
function activateRecovery(item){if(!item)return;const existing=getActivities().find(activity=>!activity.done&&activity.client===item.client&&/recuper|retomar|acompanhar/i.test(activity.title));if(existing){activeGrowthTab='recovery';showView('activities');return}const activities=getActivities(),date=new Date();date.setDate(date.getDate()+1);activities.push({id:`activity-recovery-${Date.now()}`,title:`Recuperar · ${item.title}`,type:'Recuperação',client:item.client,date:date.toISOString().slice(0,10),time:'09:00',note:recoveryMessage(item),owner:appState.currentUser?.name||'',done:false,recoverySource:item.id});saveActivities(activities);const deals=getDeals(),deal=item.deal||deals.find(entry=>entry.id===item.proposal?.dealId);if(deal){const stored=deals.find(entry=>entry.id===deal.id);stored.next=`Retomar negociação · ${item.title}`;stored.nextDate=date.toISOString().slice(0,10);addEntityHistory(stored,'Recuperação de receita iniciada',item.reason);saveDeals(deals)}renderRecoveryCenter()}
function renderWinningLibrary(){const cases=buildWinningLibrary(getDeals(),getClients()),search=String($('#winningSearch')?.value||'').toLowerCase(),visible=cases.filter(item=>`${item.clientName} ${item.segment} ${item.pain} ${item.objection}`.toLowerCase().includes(search));$('#winningLibrary').innerHTML=visible.length?visible.map(item=>`<article class="winning-card"><header><span>${escapeHtml(item.clientName.charAt(0))}</span><div><small>${escapeHtml(item.segment)} · ${item.cycle} dias de ciclo</small><h3>${escapeHtml(item.title)} · ${escapeHtml(item.clientName)}</h3></div><strong>${formatMoney(item.value)}</strong></header><div><section><small>DOR QUE MOVIMENTOU</small><p>${escapeHtml(item.pain)}</p></section><section><small>OBJEÇÃO SUPERADA</small><p>${escapeHtml(item.objection)}</p></section><section><small>MOVIMENTO VENCEDOR</small><p>${escapeHtml(item.winningMove)}</p></section></div><footer><span>Memória ${item.score}% completa</span><button type="button" data-winning-deal="${escapeHtml(item.id)}">Abrir venda</button></footer></article>`).join(''):orbitEmptyState('A biblioteca crescerá com suas vendas','Registre negócios ganhos e suas evidências para transformar experiência em método.');$('#winningSearch').oninput=renderWinningLibrary;document.querySelectorAll('[data-winning-deal]').forEach(button=>button.onclick=()=>openDealDrawer(button.dataset.winningDeal))}
function renderValueDelivery(){const value=buildAdoptionAndValue({company:getCompany()||{},clients:getClients(),deals:getDeals(),activities:getActivities(),proposals:getProposals(),conversations:getOrbitConversations()});$('#adoptionProgress').textContent=`${value.progress}%`;$('#adoptionCount').textContent=`${value.done}/${value.total}`;$('#valueHeadline').textContent=value.headline;$('#valueProtected').textContent=formatMoney(value.metrics.protectedRevenue);$('#valueHours').textContent=`${value.metrics.hours}h`;$('#valueActions').textContent=value.metrics.actions;$('#valueConversations').textContent=value.metrics.conversations;$('#adoptionChecklist').innerHTML=value.steps.map((step,index)=>`<article class="${step.done?'done':''}"><span>${step.done?'✓':index+1}</span><div><strong>${escapeHtml(step.label)}</strong><small>${step.done?'Concluído':'Próximo marco recomendado'}</small></div></article>`).join('')}
function renderValueProof(){const deals=dealsVisibleToCurrentUser(getDeals()),activities=getActivities(),proposals=getProposals(),open=deals.filter(item=>!['won','lost'].includes(item.status)),protectedDeals=open.filter(item=>item.next&&item.nextDate),protectedValue=protectedDeals.reduce((sum,item)=>sum+Number(item.value||0),0),completed=activities.filter(item=>item.done).length,conversations=getOrbitConversations().length,advanced=deals.filter(item=>(item.history||[]).some(event=>event.eventType==='stage_change')).length,viewed=proposals.filter(item=>item.status==='viewed'||Number(item.viewCount)>0).length;let section=$('#valueProofStrip');if(!section){document.querySelector('#todayView .metrics').insertAdjacentHTML('afterend','<section id="valueProofStrip" class="value-proof-strip"></section>');section=$('#valueProofStrip')}section.innerHTML=`<header><div><small>INDICADORES DE USO DO NIVIONTECH</small><strong>Evidências registradas pela sua operação.</strong></div><span>Atualizado com os dados disponíveis</span></header><div><article><strong>${formatMoney(protectedValue)}</strong><small>pipeline com próximo passo definido</small></article><article><strong>${advanced}</strong><small>negociações movimentadas</small></article><article><strong>${completed}</strong><small>ações concluídas</small></article><article><strong>${conversations}</strong><small>conversas registradas como memória</small></article><article><strong>${viewed}</strong><small>propostas com interação registrada</small></article></div>`}
let pendingCsvImport=[];
function renderProductivityWorkspace(){const select=$('#quickCaptureClient'),selected=select.value;select.innerHTML='<option value="">Selecione um cliente</option>'+getClients().map(client=>`<option value="${escapeHtml(client.id)}">${escapeHtml(client.name)}</option>`).join('');select.value=selected;$('#csvImportFile').onchange=previewCsvImport;$('#confirmCsvImport').onclick=confirmCsvImport;$('#saveQuickCapture').onclick=saveQuickCapture;$('#startVoiceCapture').onclick=startVoiceCapture;$('#savedViews').innerHTML=defaultSavedViews.map(view=>{const count=filterDealsForView(dealsVisibleToCurrentUser(getDeals()).filter(item=>!['won','lost'].includes(item.status)),view.id,{today:todayISO(),owner:appState.currentUser?.name||'',stages:pipelineStages}).length;return`<button type="button" data-saved-view="${view.id}"><span>${count}</span><div><strong>${escapeHtml(view.name)}</strong><small>${escapeHtml(view.description)}</small></div><b>→</b></button>`}).join('');document.querySelectorAll('[data-saved-view]').forEach(button=>button.onclick=()=>{appState.activePipelineView=button.dataset.savedView;showView('pipeline')})}
async function previewCsvImport(event){const file=event.target.files?.[0],box=$('#csvImportPreview');pendingCsvImport=[];$('#confirmCsvImport').disabled=true;if(!file)return;if(file.size>3*1024*1024){box.innerHTML='<p>O arquivo deve ter no máximo 3 MB.</p>';return}pendingCsvImport=parseCsv(await file.text());box.innerHTML=pendingCsvImport.length?`<strong>${pendingCsvImport.length} registros encontrados</strong><p>Colunas reconhecidas: ${escapeHtml(Object.keys(pendingCsvImport[0]).join(', '))}</p><div>${pendingCsvImport.slice(0,3).map(row=>`<span>${escapeHtml(Object.values(row).slice(0,4).join(' · '))}</span>`).join('')}</div>`:'<p>Não encontramos linhas válidas. Use a primeira linha como cabeçalho.</p>';$('#confirmCsvImport').disabled=!pendingCsvImport.length}
function csvValue(row,...keys){for(const key of keys){if(row[key])return row[key]}return''}
function confirmCsvImport(){const type=$('#csvImportType').value;if(type==='clients'){const clients=getClients();let added=0;pendingCsvImport.forEach(row=>{const name=csvValue(row,'nome','empresa','cliente');if(!name||clients.some(item=>item.name.trim().toLowerCase()===name.trim().toLowerCase()))return;clients.push({id:`client-import-${Date.now()}-${added}`,name,segment:csvValue(row,'segmento'),status:csvValue(row,'status')||'Prospect',city:csvValue(row,'cidade'),phone:csvValue(row,'telefone','celular','whatsapp'),email:csvValue(row,'email','e-mail'),owner:appState.currentUser?.name||'',createdAt:new Date().toISOString(),source:'Importação CSV'});added++});saveClients(clients);$('#csvImportPreview').innerHTML=`<strong>${added} clientes importados ✓</strong>`}else{const deals=getDeals();let added=0;pendingCsvImport.forEach(row=>{const title=csvValue(row,'oportunidade','titulo','negocio'),client=csvValue(row,'cliente','empresa');if(!title||!client)return;deals.push({id:`deal-import-${Date.now()}-${added}`,title,client,value:Number(String(csvValue(row,'valor')).replace(/[^0-9,.-]/g,'').replace(',','.'))||0,stage:stageByMeaning('new'),owner:csvValue(row,'responsavel')||appState.currentUser?.name||'',next:csvValue(row,'proximo_passo','proxima_acao')||'Revisar oportunidade importada',nextDate:csvValue(row,'data','data_da_proxima_acao')||todayISO(),status:'open',createdAt:new Date().toISOString(),source:'Importação CSV'});added++});saveDeals(deals);$('#csvImportPreview').innerHTML=`<strong>${added} oportunidades importadas ✓</strong>`}pendingCsvImport=[];$('#confirmCsvImport').disabled=true;renderProductivityWorkspace()}
function saveQuickCapture(){const client=getClients().find(item=>item.id===$('#quickCaptureClient').value),text=$('#quickCaptureText').value.trim();if(!client||!text){alert('Selecione um cliente e registre uma anotação.');return}const clients=getClients(),stored=clients.find(item=>item.id===client.id);stored.interactions=stored.interactions||[];stored.interactions.unshift({id:`interaction-mobile-${Date.now()}`,title:'Captura rápida',text,source:'Captura móvel',date:new Date().toISOString()});addEntityHistory(stored,'Captura rápida registrada',text.slice(0,140));saveClients(clients);$('#quickCaptureText').value='';$('#voiceCaptureStatus').textContent='Anotação salva na memória do cliente ✓'}
function startVoiceCapture(){const Recognition=window.SpeechRecognition||window.webkitSpeechRecognition;if(!Recognition){$('#voiceCaptureStatus').textContent='Ditado não disponível neste navegador. Você pode digitar normalmente.';return}const recognition=new Recognition();recognition.lang='pt-BR';recognition.interimResults=false;recognition.onstart=()=>$('#voiceCaptureStatus').textContent='Ouvindo... fale o resumo do atendimento.';recognition.onresult=event=>{$('#quickCaptureText').value=[ $('#quickCaptureText').value,event.results[0][0].transcript].filter(Boolean).join(' ');$('#voiceCaptureStatus').textContent='Ditado capturado. Revise antes de salvar.'};recognition.onerror=()=>$('#voiceCaptureStatus').textContent='Não foi possível ouvir. Tente novamente ou digite a anotação.';recognition.start()}
function renderGrowthOs(){const cockpit=buildRevenueCockpit(getDeals(),getActivities(),pipelineStages,todayISO());$('#growthWeighted').textContent=formatMoney(cockpit.weighted);document.querySelectorAll('[data-growth-tab]').forEach(button=>{button.classList.toggle('active',button.dataset.growthTab===activeGrowthTab);button.onclick=()=>{activeGrowthTab=button.dataset.growthTab;renderGrowthOs()}});document.querySelectorAll('[data-growth-panel]').forEach(panel=>panel.classList.toggle('hidden',panel.dataset.growthPanel!==activeGrowthTab));renderGrowthSignals(cockpit);renderGrowthAutomations();renderGrowthCoaching();renderGrowthExecutive(cockpit);renderIcpRadar();renderInfluenceMap();renderConversationIntelligence();renderRevenueLeaks();renderGrowthMissions();renderCommercialTruth();renderFunnelVelocity();renderDailyCommand();renderRecoveryCenter();renderWinningLibrary();renderValueDelivery();renderProductivityWorkspace()}
...
function renderPipeline(search=''){
  const pipelineView=$('#pipelineView'),pipelineScrollRoot=$('#appScreen .workspace'),kanbanScroller=pipelineView?.querySelector('.kanban-scroll');
  const preservedScroll={top:pipelineScrollRoot?.scrollTop||0,left:kanbanScroller?.scrollLeft||0};
  ...
  const visible=focused.filter(deal=>(deal.title+' '+deal.client).toLowerCase().includes(search.toLowerCase()));
  ...
  const pipelineTotal=activeDeals.reduce((sum,deal)=>sum+Number(deal.value||0),0);
  const probabilityFor=deal=>{const index=Math.max(0,commercialStages.findIndex(stage=>stage.id===deal.stage));return Math.round(15+(index/Math.max(1,commercialStages.length-1))*70)};
  const weightedTotal=activeDeals.reduce((sum,deal)=>sum+Number(deal.value||0)*probabilityFor(deal)/100,0);
  const riskDeals=activeDeals.filter(deal=>{const health=healthForDeal(deal);return health.tone==='risk'||!deal.next||!deal.nextDate||(deal.nextDate<todayISO())});
  const riskValue=riskDeals.reduce((sum,deal)=>sum+Number(deal.value||0),0);
  const priorityId=[...riskDeals].sort((a,b)=>Number(b.value||0)-Number(a.value||0))[0]?.id||'';
  $('#pipelineTotal').textContent=formatMoney(pipelineTotal);
  $('#pipelineProbability').textContent=pipelineTotal?`${Math.round(weightedTotal/pipelineTotal*100)}%`:'0%';
  $('#pipelineRiskValue').textContent=formatMoney(riskValue);
  $('#pipelineRiskCount').textContent=`${riskDeals.length} ${riskDeals.length===1?'negócio':'negócios'}`;
  $('#pipelineOpenLabel').textContent=`${activeDeals.length} ${activeDeals.length===1?'negócio ativo':'negócios ativos'}`;
  $('#dealCount').textContent=activeDeals.length;
  let focus=$('#pipelineSavedViewFocus');if(appState.activePipelineView){const view=defaultSavedViews.find(item=>item.id===appState.activePipelineView);if(!focus){document.querySelector('.pipeline-toolbar').insertAdjacentHTML('afterend','<div id="pipelineSavedViewFocus" class="pipeline-saved-focus"></div>');focus=$('#pipelineSavedViewFocus')}focus.innerHTML=`<span>VISÃO SALVA</span><strong>${escapeHtml(view?.name||'Foco personalizado')}</strong><small>${visible.length} resultados</small><button type="button">Limpar visão</button>`;focus.querySelector('button').onclick=()=>{appState.activePipelineView=null;renderPipeline($('#dealSearch').value)}}else focus?.remove();
  ...
  requestAnimationFrame(()=>{
    if(pipelineScrollRoot&&pipelineView&&!pipelineView.classList.contains('hidden'))pipelineScrollRoot.scrollTop=preservedScroll.top;
...
function dealCard(deal,priorityId=''){const health=healthForDeal(deal),contactAge=daysWithoutContactLabel(deal.client,deal.createdAt),checklist=stageChecklistForDeal(deal,pipelineStages),temperature=health.tone==='healthy'?'Quente':health.tone==='attention'?'Morno':'Em risco',isMoved=appState.pipelineMovedDealId===deal.id;return `<article class="deal-card health-${health.tone} ${deal.id===priorityId?'deal-priority':''} ${isMoved?'deal-just-moved':''} ${deal.status==='lost'?'lost':deal.status==='won'?'won':''}" draggable="true" data-deal-id="${escapeHtml(deal.id)}"><div class="deal-top"><h4>${escapeHtml(deal.title)}</h4><button type="button" data-open-deal="${escapeHtml(deal.id)}" title="Abrir ficha">•••</button></div><p class="deal-client">${escapeHtml(deal.client)}</p>${deal.distributionReason?`<small class="deal-assignment-reason">↻ ${escapeHtml(deal.distributionReason)}</small>`:''}<div class="deal-value-line"><strong class="deal-value">${formatMoney(deal.value)}</strong><span class="deal-temperature health-${health.tone}" title="${escapeHtml(health.reason)}"><i></i>${temperature}</span></div><div class="deal-readiness"><span><i style="width:${checklist.percent}%"></i></span><small>${checklist.percent}% da negociação</small></div><div class="deal-next ${deal.next&&deal.nextDate?'':'missing'}"><small>MELHOR PRÓXIMO PASSO · ${deal.nextDate?deal.nextDate===todayISO()?'HOJE':formatDate(deal.nextDate):'SEM DATA'}</small><p>${escapeHtml(deal.next||'Defina uma ação concreta')}</p></div><footer class="deal-footer"><span class="owner-avatar" title="${escapeHtml(deal.owner)}">${ownerInitials(deal.owner)}</span><span class="deal-owner-name">${escapeHtml(deal.owner||'Sem responsável')}</span><span class="contact-age" title="Tempo desde a última interação">${escapeHtml(contactAge)}</span><span class="move-hint">${daysSince(deal.movedAt||deal.createdAt)}d na etapa</span><select class="card-move" data-move-deal="${escapeHtml(deal.id)}" aria-label="Mover ${escapeHtml(deal.title)} para outra etapa"><option value="">Mover para...</option>${pipelineStages.filter(stage=>stage.id!==deal.stage).map(stage=>`<option value="${escapeHtml(stage.id)}">${escapeHtml(stage.label)}</option>`).join('')}</select></footer></article>`}
...
function dealNarrativeFor(deal,{health,checklist,activities,proposals,client}){
  const stage=pipelineStages.find(item=>item.id===deal.stage)?.label||'etapa atual',missing=checklist.items.filter(item=>!item.done).map(item=>item.label),proposal=proposals[0],overdue=deal.nextDate&&deal.nextDate<todayISO(),inactive=daysWithoutContact(deal.client,deal.updatedAt||deal.movedAt||deal.createdAt),evidence=[];
  if(deal.pain||deal.orbitMemory?.pains?.[0])evidence.push('dor comercial confirmada');
  if(deal.decisionMaker||deal.orbitMemory?.decisionMakers?.[0])evidence.push('decisor identificado');
  if(proposal)evidence.push(`${proposal.status==='viewed'?'proposta visualizada':'proposta vinculada'}${proposal.viewCount?` ${proposal.viewCount}x`:''}`);
  if(deal.next&&deal.nextDate)evidence.push(`próximo passo em ${formatDate(deal.nextDate)}`);
  if(!evidence.length)evidence.push('ainda há poucas evidências registradas');
  let action=deal.next||'Definir próximo passo e data',command='focus-next';
  if(overdue){action=`Reagendar: ${deal.next||'próximo passo'}`;command='focus-next'}
  else if(!deal.decisionMaker){action='Identificar quem participa da decisão';command='focus-decision'}
  else if(proposal&&['sent','viewed'].includes(proposal.status)){action='Retomar a decisão sobre a proposta';command='proposals'}
  else if(!deal.orbitMemory?.summary){action='Preparar a próxima conversa com o Orbit';command='prepare'}
  const riskTone=health.score<45?'risk':health.score<70?'attention':'good',riskLabel=health.score<45?'Risco alto':health.score<70?'Pede atenção':'Bem conduzida';
  const situation=`${deal.client} está em ${stage.toLowerCase()} com saúde ${health.label.toLowerCase()}. ${inactive===null?'Ainda não há contato registrado.':inactive===0?'Houve interação hoje.':`A última evidência de contato tem ${inactive} ${inactive===1?'dia':'dias'}.`}`;
  const impact=missing.length?`${formatMoney(deal.value)} dependem de ${missing.slice(0,2).join(' e ').toLowerCase()} para ganhar confiança.`:`${formatMoney(deal.value)} estão com os critérios atuais completos para esta etapa.`;
  return{title:health.score<70?'Esta negociação precisa de condução':'A negociação está pronta para continuar',tone:riskTone,label:riskLabel,situation,evidence:evidence.join(' · ')+'.',impact,action,command};
}
function renderDealNarrative(deal,context){
  const narrative=dealNarrativeFor(deal,context),tone=$('#dealNarrativeTone');
  $('#dealNarrativeTitle').textContent=narrative.title;$('#dealNarrativeSituation').textContent=narrative.situation;$('#dealNarrativeEvidence').textContent=narrative.evidence;$('#dealNarrativeImpact').textContent=narrative.impact;$('#dealNarrativeAction').textContent=narrative.action;tone.textContent=narrative.label;tone.className=`deal-narrative-tone ${narrative.tone}`;
  $('#executeDealNarrative').onclick=()=>{if(narrative.command==='focus-next'){$('#drawerDealNext').focus();$('#drawerDealNext').scrollIntoView({behavior:'smooth',block:'center'})}else if(narrative.command==='focus-decision'){$('#drawerDealDecisionMaker').focus();$('#drawerDealDecisionMaker').scrollIntoView({behavior:'smooth',block:'center'})}else if(narrative.command==='proposals'){closeDealDrawer();showView('proposals')}else{$('#prepareActiveDeal').click()}};
}
function ensureClosingPlanForDeal(deal){const plans=getClosingPlans();let plan=plans.find(item=>item.dealId===deal.id&&!mutualPlanProgress(item).complete);if(plan)return plan;plan=createMutualActionPlan({deal,client:clientForDeal(deal)});plans.unshift(plan);saveClosingPlans(plans);addEntityHistory(deal,'Plano de fechamento criado',`${plan.milestones.length} compromissos organizados até ${plan.targetDate}`);const deals=getDeals(),stored=deals.find(item=>item.id===deal.id);stored.history=deal.history;saveDeals(deals);return plan}
function openDealProposalFlow(deal){closeDealDrawer();showView('proposals');const proposal=getProposals().find(item=>item.dealId===deal.id);if(proposal){openProposalPreview(proposal.id);return}openProposalModal();$('#proposalDeal').value=deal.id;$('#proposalDeal').dispatchEvent(new Event('change'))}
function renderDealJourney(deal,{proposals,activities}){
  let section=$('#dealJourneyWorkspace');if(!section){$('#dealNarrative').insertAdjacentHTML('afterend','<section id="dealJourneyWorkspace" class="deal-journey"><header><div><small>JORNADA DA NEGOCIAÇÃO</small><h3>Conduza tudo por aqui</h3></div><span>Contexto preservado</span></header><div id="dealJourneySteps" class="deal-journey-steps"></div><div id="dealValueEvolution" class="deal-value-evolution"></div></section>');section=$('#dealJourneyWorkspace')}
  const proposal=proposals[0],plan=getClosingPlans().find(item=>item.dealId===deal.id),planProgress=plan?mutualPlanProgress(plan):null,room=proposal?.dealRoom,received=deal.paymentStatus==='received',managerialView=canManageReceipts(),completedActivities=getActivities().filter(item=>item.client===deal.client&&item.done).length,history=deal.history||[],stageMoves=history.filter(item=>item.eventType==='stage_change').length,score=Number(deal.orbitMemory?.qualificationScore||0);
  const steps=[
    {key:'conversation',label:'Conversa',state:deal.orbitMemory?.summary?'Memória atualizada':'Análise pendente',done:Boolean(deal.orbitMemory?.summary),action:'analyze'},
    {key:'action',label:'Próxima ação',state:deal.next&&deal.nextDate?`${deal.next} · ${formatDate(deal.nextDate)}`:'Definir continuidade',done:Boolean(deal.next&&deal.nextDate),action:'next'},
    {key:'proposal',label:'Proposta',state:proposal?`${proposalStatusLabel(proposal.status)} · ${formatMoney(proposal.value)}`:'Ainda não criada',done:Boolean(proposal),action:'proposal'},
    {key:'plan',label:'Fechamento',state:planProgress?`${planProgress.percent}% concluído`:'Criar plano compartilhado',done:Boolean(plan),action:'plan'},
    {key:'room',label:'Sala segura',state:room?'Acesso criado':'Compartilhar decisão',done:Boolean(room),action:'room'},
    {key:'money',label:managerialView?'Recebimento':'Resultado',state:managerialView?(received?`${formatMoney(deal.receivedAmount||deal.value)} recebido`:deal.status==='won'?'Venda ganha · receber':'Em negociação'):(deal.status==='won'?'Venda ganha':deal.status==='lost'?'Venda perdida':'Em negociação'),done:managerialView?received:['won','lost'].includes(deal.status),action:'money'}
  ];
  $('#dealJourneySteps').innerHTML=steps.map((step,index)=>`<button type="button" class="deal-journey-step ${step.done?'done':''}" data-journey-action="${step.action}"><span>${step.done?'✓':index+1}</span><div><small>${step.label}</small><strong>${escapeHtml(step.state)}</strong></div><b>→</b></button>`).join('');
  $('#dealValueEvolution').innerHTML=`<div><small>VALOR PRODUZIDO NESTA NEGOCIAÇÃO</small><strong>${score?`Qualificação em ${score}%`:'Evolução em construção'}</strong></div><span>${stageMoves} avanços registrados</span><span>${completedActivities} ações concluídas</span><span>${deal.decisionMaker?'Decisor mapeado':'Decisor pendente'}</span><span>${deal.next&&deal.nextDate?'Continuidade protegida':'Continuidade pendente'}</span>`;
  section.querySelectorAll('[data-journey-action]').forEach(button=>button.onclick=()=>{const action=button.dataset.journeyAction;if(action==='analyze')$('#analyzeActiveDeal').click();else if(action==='next'){$('#drawerDealNext').focus();$('#drawerDealNext').scrollIntoView({behavior:'smooth',block:'center'})}else if(action==='proposal')openDealProposalFlow(deal);else if(action==='plan'){const created=ensureClosingPlanForDeal(deal);activeClosingPlanId=created.id;closeDealDrawer();showView('closingPlans')}else if(action==='room'){if(!proposal){openDealProposalFlow(deal);return}closeDealDrawer();showView('proposals');openProposalPreview(proposal.id);setTimeout(openDealRoomCreator,0)}else if(action==='money'){if(managerialView&&deal.status==='won')openReceiptModal(deal.id);else $('#markWon').scrollIntoView({behavior:'smooth',block:'center'})}})
}
function renderDealCockpit(deal,{health,checklist,activities,proposals}){
  const commercialStages=commercialPipelineStages(pipelineStages),stageIndex=Math.max(0,commercialStages.findIndex(stage=>stage.id===deal.stage)),probability=Math.round(15+(stageIndex/Math.max(1,commercialStages.length-1))*70),forecast=Number(deal.value||0)*probability/100,daysToAction=deal.nextDate?Math.ceil((new Date(`${deal.nextDate}T12:00:00`).getTime()-new Date(`${todayISO()}T12:00:00`).getTime())/86400000):null,isOverdue=daysToAction!==null&&daysToAction<0,proposal=proposals[0],evidenceCount=checklist.done+(deal.orbitMemory?.summary?1:0)+(proposal?1:0),stageLabel=pipelineStages.find(stage=>stage.id===deal.stage)?.label||'Em negociação',tone=health.tone==='healthy'?'healthy':health.tone;
  const deadlineLabel=daysToAction===null?'Sem data':isOverdue?`${Math.abs(daysToAction)}d atrasado`:daysToAction===0?'Hoje':`${daysToAction}d restantes`;
  $('#dealCockpit').innerHTML=`<header><div><small>COCKPIT DA NEGOCIAÇÃO</small><strong>${escapeHtml(stageLabel)}</strong></div><span class="cockpit-health ${tone}"><i></i>${escapeHtml(health.label)} · ${health.score}/100</span></header><div class="deal-cockpit-metrics"><article><small>VALOR</small><strong>${formatMoney(deal.value)}</strong><span>potencial comercial</span></article><article><small>PREVISÃO</small><strong>${formatMoney(forecast)}</strong><span>${probability}% de probabilidade</span></article><article class="${isOverdue?'risk':''}"><small>PRÓXIMA AÇÃO</small><strong>${escapeHtml(deadlineLabel)}</strong><span>${escapeHtml(deal.next||'Ação não definida')}</span></article><article><small>EVIDÊNCIAS</small><strong>${evidenceCount}</strong><span>${checklist.percent}% de prontidão</span></article></div><div class="deal-cockpit-signals"><span class="${deal.decisionMaker?'ready':'missing'}">${deal.decisionMaker?'✓ Decisor identificado':'! Decisor pendente'}</span><span class="${proposal?'ready':'neutral'}">${proposal?'✓ Proposta vinculada':'○ Sem proposta'}</span><span class="${activities.length?'ready':'neutral'}">${activities.length?`✓ ${activities.length} ${activities.length===1?'ação pendente':'ações pendentes'}`:'○ Agenda livre'}</span><span class="${deal.orbitMemory?.summary?'ready':'neutral'}">${deal.orbitMemory?.summary?'✓ Memória atualizada':'○ Conversa não analisada'}</span></div><footer><button type="button" data-cockpit-action="next">Definir próximo passo</button><button type="button" data-cockpit-action="prepare">Preparar reunião</button><button type="button" data-cockpit-action="analyze">Analisar conversa</button><button type="button" class="primary" data-cockpit-action="proposal">${proposal?'Abrir proposta':'Criar proposta'}</button></footer>`;
  $('#dealCockpit').querySelectorAll('[data-cockpit-action]').forEach(button=>button.onclick=()=>{const action=button.dataset.cockpitAction;if(action==='next'){$('#drawerDealNext').focus();$('#drawerDealNext').scrollIntoView({behavior:'smooth',block:'center'})}else if(action==='prepare')$('#prepareActiveDeal').click();else if(action==='analyze')$('#analyzeActiveDeal').click();else openDealProposalFlow(deal)});
}
function openDealDrawer(id){
  ...
  $('#markWon').disabled=won||lost;$('#markWon').textContent=won?'Venda ganha ✓':'Marcar como ganha';$('#markLost').disabled=won||lost;$('#markLost').textContent=lost?'Venda perdida ✓':'Marcar como perdida';$('#markReceived').classList.toggle('hidden',!canManageReceipts());$('#markReceived').disabled=!won||received;$('#markReceived').textContent=received?'Recebido ✓':'Confirmar recebimento';$('#paymentBadge').classList.toggle('hidden',!canManageReceipts());
  ...
  renderDealCockpit(deal,{health,checklist,activities,proposals});
  ...
  renderDealNarrative(deal,{health,checklist,activities,proposals,client});
  renderDealJourney(deal,{proposals,activities});
...
const catalogStore=createStore(CATALOG_KEY,[]);
function getClients(){return clientsStore.get()}
function saveClients(clients){clientsStore.save(clients)}
function getCatalog(){return catalogStore.get()}
function saveCatalog(items){catalogStore.save(items)}
function catalogTypeLabel(type){return({service:'Serviço',product:'Produto',plan:'Plano'})[type]||'Item'}
function clientProductStatusLabel(status){return({interest:'Interesse',contracted:'Contratado',renewed:'Renovado',cancelled:'Encerrado'})[status]||status}
function renderCatalog(){const search=($('#catalogSearch')?.value||'').trim().toLowerCase(),items=getCatalog(),visible=items.filter(item=>(item.name+' '+item.category+' '+item.description).toLowerCase().includes(search));$('#catalogActiveCount').textContent=items.filter(item=>item.status==='active').length;$('#catalogList').innerHTML=visible.length?visible.map(item=>`<article class="catalog-item ${item.status}"><span>${escapeHtml(item.name.charAt(0).toUpperCase())}</span><div><strong>${escapeHtml(item.name)}</strong><p>${escapeHtml(catalogTypeLabel(item.type))}${item.category?` · ${escapeHtml(item.category)}`:''}</p><small>${escapeHtml(item.description||'Sem descrição comercial')}</small></div><b>${item.price?formatMoney(item.price):'Preço livre'}</b><button type="button" data-toggle-catalog="${escapeHtml(item.id)}">${item.status==='active'?'Desativar':'Ativar'}</button></article>`).join(''):orbitEmptyState('Seu catálogo começa simples','Cadastre os produtos, serviços ou planos que sua empresa oferece.');document.querySelectorAll('[data-toggle-catalog]').forEach(button=>button.onclick=()=>{const records=getCatalog(),item=records.find(record=>record.id===button.dataset.toggleCatalog);if(!item)return;item.status=item.status==='active'?'inactive':'active';item.updatedAt=new Date().toISOString();saveCatalog(records);renderCatalog()});requestAnimationFrame(syncPremiumSelects)}
...
function completeClientRegistration(client){const clients=getClients();client.id='client-'+Date.now();client.owner=appState.currentUser?.name||'';client.interactions=[];client.history=[];clients.push(client);saveClients(clients);closeClientMergeModal();closeClientModal();showView('clients')}
...
function renderClientCommandCenter(client,{deals,activities,pending,relationship}){
  const openDeals=deals.filter(deal=>!['won','lost'].includes(deal.status)),pipeline=openDeals.reduce((sum,deal)=>sum+Number(deal.value||0),0),won=deals.filter(deal=>deal.status==='won'),ordered=pending.slice().sort((a,b)=>`${a.date}${a.time||''}`.localeCompare(`${b.date}${b.time||''}`)),next=ordered[0],overdue=ordered.filter(item=>item.date<todayISO()).length,daysLabel=daysWithoutContactLabel(client.name,client.createdAt),health=overdue?'attention':relationship.percent<50?'risk':'healthy';
  const situation=overdue?`${overdue} ${overdue===1?'compromisso vencido exige':'compromissos vencidos exigem'} atenção.`:openDeals.length?`${openDeals.length} ${openDeals.length===1?'negociação ativa':'negociações ativas'} somando ${formatMoney(pipeline)}.`:won.length?'Cliente conquistado, agora o foco é retenção e expansão.':'Relacionamento ainda sem oportunidade ativa.';
  const action=next?`${next.title} · ${formatDate(next.date)} às ${next.time||'09:00'}`:openDeals[0]?`Definir o próximo compromisso de ${openDeals[0].title}.`:'Registrar uma nova interação e identificar a próxima oportunidade.';
  $('#clientCommandCenter').innerHTML=`<header><div><span class="client-command-orbit">O</span><div><small>ORBIT · LEITURA DA CONTA</small><strong>${escapeHtml(situation)}</strong><p>${escapeHtml(action)}</p></div></div><b class="${health}">${health==='healthy'?'Conta acompanhada':health==='attention'?'Ação necessária':'Contexto incompleto'}</b></header><div class="client-command-metrics"><article><small>PIPELINE ATIVO</small><strong>${formatMoney(pipeline)}</strong><span>${openDeals.length} ${openDeals.length===1?'oportunidade':'oportunidades'}</span></article><article><small>RELACIONAMENTO</small><strong>${relationship.percent}%</strong><span>mapa comercial preenchido</span></article><article><small>ÚLTIMO CONTATO</small><strong>${escapeHtml(daysLabel)}</strong><span>${pending.length} ações pendentes</span></article><article><small>PRÓXIMA AÇÃO</small><strong>${escapeHtml(next?.title||'Definir contato')}</strong><span>${next?formatDate(next.date):'Sem data definida'}</span></article></div><footer><button type="button" data-client-command="interaction">Registrar interação</button><button type="button" data-client-command="deal" ${openDeals.length?'':'disabled'}>Abrir negociação</button><button type="button" data-client-command="activity">Criar atividade</button><button type="button" class="primary" data-client-command="followup">Acompanhar conta</button></footer>`;
  const center=$('#clientCommandCenter');
  center.querySelector('[data-client-command="interaction"]').onclick=()=>{$('#interactionForm').scrollIntoView({behavior:'smooth',block:'center'});$('#interactionForm input').focus()};
  center.querySelector('[data-client-command="deal"]').onclick=()=>{if(!openDeals[0])return;closeClientDrawer();openDealDrawer(openDeals[0].id)};
  center.querySelector('[data-client-command="activity"]').onclick=()=>{closeClientDrawer();openActivityModal();const field=$('#activityForm [name="client"]');if(field)field.value=client.name};
  center.querySelector('[data-client-command="followup"]').onclick=()=>{createSuccessFollowUp(client.id);openClientDrawer(client.id)};
}
function openClientDrawer(id){
  ...
  renderClientCommandCenter(client,{deals,activities,pending,relationship});
  ...
  const catalog=getCatalog(),clientProducts=client.products||[],catalogOptions=catalog.filter(item=>item.status==='active').map(item=>`<option value="${escapeHtml(item.id)}">${escapeHtml(item.name)}${item.price?` · ${formatMoney(item.price)}`:''}</option>`).join('');$('#clientProductCatalog').innerHTML=catalogOptions?`<option value="">Selecione</option>${catalogOptions}`:'<option value="">Cadastre um item em Gestão</option>';$('#clientProductForm [name="date"]').value=todayISO();$('#clientProductsList').innerHTML=clientProducts.length?clientProducts.slice().sort((a,b)=>String(b.date).localeCompare(String(a.date))).map(record=>{const item=catalog.find(entry=>entry.id===record.catalogId);return`<article class="client-product-item"><span>${escapeHtml((item?.name||record.name||'I').charAt(0))}</span><div><strong>${escapeHtml(item?.name||record.name||'Item comercial')}</strong><p>${escapeHtml(clientProductStatusLabel(record.status))} · ${Number(record.quantity||1)} un. · ${formatDate(record.date)}</p><small>${escapeHtml(record.note||'Sem observação')}</small></div><b>${record.value?formatMoney(record.value):'Valor não informado'}</b></article>`}).join(''):'<div class="history-empty">Nenhum produto ou serviço registrado para este cliente.</div>';
  const productTimeline=clientProducts.map(record=>{const item=catalog.find(entry=>entry.id===record.catalogId);return{id:record.id,title:`${clientProductStatusLabel(record.status)} · ${item?.name||record.name||'Item comercial'}`,text:[record.value?formatMoney(record.value):'',record.note].filter(Boolean).join(' · ')||'Registro comercial',date:`${record.date}T12:00:00`,kind:'Produto'}});
  const timeline=[...(client.interactions||[]).map(item=>({...item,kind:'Interação'})),...productTimeline,...activities.map(item=>({id:item.id,title:item.done?'Atividade concluída':'Atividade registrada',text:`${item.type}: ${item.title}`,date:item.completedAt||`${item.date}T${item.time||'12:00'}`,kind:'Atividade'})),...deals.flatMap(deal=>(deal.history||[]).map(item=>({...item,kind:'Negociação'})))].sort((a,b)=>new Date(b.date)-new Date(a.date));
...
function activityCard(activity){const client=getClients().find(item=>item.name.trim().toLowerCase()===String(activity.client||'').trim().toLowerCase());return `<article class="agenda-item ${activity.done?'done':''}"><button class="complete-activity" data-complete-activity="${activity.id}" title="${activity.done?'Reabrir':'Concluir'}">${activity.done?'✓':'○'}</button><div><strong>${escapeHtml(activity.title)}</strong><p>${escapeHtml(activity.client)} · ${escapeHtml(activity.note||'Sem observação')}</p><span class="activity-type">${escapeHtml(activity.type)}</span></div><time>${activity.date===todayISO()?'Hoje':formatDate(activity.date)}<br><b>${activity.time}</b></time><div class="activity-actions"><button type="button" data-activity-menu="${activity.id}" aria-label="Ações da atividade">•••</button><div><button data-activity-action="whatsapp" data-activity-id="${activity.id}" ${client?.phone||client?.whatsapp?'':'disabled'}>WhatsApp preparado</button><button data-activity-action="email" data-activity-id="${activity.id}" ${client?.email?'':'disabled'}>E-mail preparado</button><button data-activity-action="snooze" data-days="1" data-activity-id="${activity.id}">Adiar 1 dia</button><button data-activity-action="snooze" data-days="3" data-activity-id="${activity.id}">Adiar 3 dias</button><button data-activity-action="complete-next" data-activity-id="${activity.id}">Concluir + próximo passo</button></div></div></article>`}
function activityMessage(activity){return `Olá! Estou entrando em contato sobre ${activity.title.toLowerCase()}. ${activity.note||'Podemos alinhar o próximo passo?'}`}
function registerActivityContact(activity,channel,message){const clients=getClients(),client=clients.find(item=>item.name.trim().toLowerCase()===String(activity.client||'').trim().toLowerCase());if(!client)return;client.interactions=client.interactions||[];client.interactions.unshift({id:`interaction-${Date.now()}`,title:`${channel} preparado · ${activity.title}`,text:message,date:new Date().toISOString()});addEntityHistory(client,`${channel} preparado`,activity.title);saveClients(clients)}
function handleActivityAction(button){const activities=getActivities(),activity=activities.find(item=>item.id===button.dataset.activityId);if(!activity)return;const action=button.dataset.activityAction;if(action==='snooze'){shiftActivity(activity,button.dataset.days);addEntityHistory(activity,'Atividade adiada',`Nova data: ${activity.date}`);saveActivities(activities);renderActivities();return}if(action==='complete-next'){openCompleteNextActivity(activity.id);return}const client=getClients().find(item=>item.name.trim().toLowerCase()===String(activity.client||'').trim().toLowerCase()),message=activityMessage(activity);registerActivityContact(activity,action==='whatsapp'?'WhatsApp':'E-mail',message);if(action==='whatsapp'){const phone=String(client?.whatsapp||client?.phone||'').replace(/\D/g,'');window.open(`https://wa.me/${phone}?text=${encodeURIComponent(message)}`,'_blank','noopener')}else window.location.href=`mailto:${client?.email||''}?subject=${encodeURIComponent(activity.title)}&body=${encodeURIComponent(message)}`}
function closeActivityActionMenus(except=null){document.querySelectorAll('.activity-actions.open').forEach(menu=>{if(menu!==except)menu.classList.remove('open')})}
function bindActivityButtons(){document.querySelectorAll('[data-complete-activity]').forEach(button=>button.onclick=()=>{const activities=getActivities();const activity=activities.find(item=>item.id===button.dataset.completeActivity);activity.done=!activity.done;activity.completedAt=activity.done?new Date().toISOString():null;addEntityHistory(activity,activity.done?'Atividade concluída':'Atividade reaberta',activity.title);saveActivities(activities);appState.currentView==='today'?renderTodayActivities():renderActivities()});document.querySelectorAll('[data-activity-menu]').forEach(button=>button.onclick=event=>{event.stopPropagation();const menu=button.parentElement,willOpen=!menu.classList.contains('open');closeActivityActionMenus();if(willOpen)menu.classList.add('open')});document.querySelectorAll('[data-activity-action]').forEach(button=>button.onclick=event=>{event.stopPropagation();closeActivityActionMenus();handleActivityAction(button)});if(!document.body.dataset.activityMenusReady){document.body.dataset.activityMenusReady='1';document.addEventListener('click',event=>{if(!event.target.closest('.activity-actions'))closeActivityActionMenus()});document.addEventListener('keydown',event=>{if(event.key==='Escape')closeActivityActionMenus()});window.addEventListener('scroll',()=>closeActivityActionMenus(),true)}}
function openCompleteNextActivity(id){const activity=getActivities().find(item=>item.id===id);if(!activity)return;let modal=$('#completeNextActivityModal');if(!modal){document.body.insertAdjacentHTML('beforeend',`<div id="completeNextActivityModal" class="modal hidden"><div class="modal-backdrop" data-close-complete-next></div><form class="modal-card"><header><div><p class="overline">RITMO COMERCIAL</p><h2>Concluir e manter o avanço</h2><small>Finalize esta atividade e deixe o próximo compromisso pronto.</small></div><button type="button" data-close-complete-next>×</button></header><div class="deal-form-grid"><label class="full-field">Próximo passo<input name="title" required></label><label>Data<input name="date" type="date" required></label><label>Horário<input name="time" type="time" value="09:00" required></label><label>Tipo<select name="type"><option>Follow-up</option><option>Ligação</option><option>Reunião</option><option>WhatsApp</option><option>E-mail</option></select></label></div><footer><button type="button" class="secondary" data-close-complete-next>Cancelar</button><button class="primary">Concluir e criar próximo passo</button></footer></form></div>`);modal=$('#completeNextActivityModal');modal.querySelectorAll('[data-close-complete-next]').forEach(button=>button.onclick=()=>modal.classList.add('hidden'));modal.querySelector('form').onsubmit=event=>{event.preventDefault();const current=getActivities(),source=current.find(item=>item.id===modal.dataset.activityId),next=Object.fromEntries(new FormData(event.currentTarget)),result=completeActivityWithNext(source,next);addEntityHistory(source,'Atividade concluída','Próximo passo criado automaticamente');if(result.nextActivity)current.push(result.nextActivity);saveActivities(current);modal.classList.add('hidden');renderActivities()}}modal.dataset.activityId=id;modal.querySelector('[name="title"]').value='Retomar '+activity.title.toLowerCase();const date=new Date();date.setDate(date.getDate()+2);modal.querySelector('[name="date"]').value=date.toISOString().slice(0,10);modal.classList.remove('hidden')}
...
function renderTodayOrbitRecommendations(){const target=$('#todayOrbitRecommendations');if(!target)return;const actions=currentNextBestActions();target.innerHTML=actions.length?actions.slice(0,3).map((item,index)=>`<button class="recommendation recommendation-action next-action-card orbit-story-card" data-next-action="${escapeHtml(item.id)}"><span>${index+1}</span><div><small>SITUAÇÃO</small><strong>${escapeHtml(item.client)} · ${escapeHtml(item.title)}</strong><p><b>Evidência:</b> ${escapeHtml(item.reason)}</p><p><b>Impacto:</b> ${item.value?`${formatMoney(item.value)} precisam de condução.`:'Esta pendência pode interromper o ritmo comercial.'}</p><em>${escapeHtml(item.action)} →</em></div></button>`).join(''):orbitEmptyState('Operação sob controle','Quando surgir risco ou compromisso vencido, o Orbit mostrará situação, evidência, impacto e ação aqui.');target.querySelectorAll('[data-next-action]').forEach(button=>button.onclick=()=>openNextBestAction(button.dataset.nextAction));renderOrbitExecution(actions)}
...
function renderTodayCommandCenter(activities,deals){
  const today=todayISO(),pending=activities.filter(item=>!item.done),overdue=pending.filter(item=>item.date<today),dueToday=pending.filter(item=>item.date===today),future=pending.filter(item=>item.date>today).sort((a,b)=>`${a.date}${a.time||''}`.localeCompare(`${b.date}${b.time||''}`)),agenda=[...overdue,...dueToday,...future].slice(0,4),linkedValue=new Set(agenda.map(item=>item.client)).size?deals.filter(deal=>agenda.some(item=>item.client===deal.client)&&!['won','lost'].includes(deal.status)).reduce((sum,deal)=>sum+Number(deal.value||0),0):0;
  const headline=overdue.length?`Seu dia começa protegendo ${overdue.length} ${overdue.length===1?'compromisso vencido':'compromissos vencidos'}.`:dueToday.length?`Você tem ${dueToday.length} ${dueToday.length===1?'ação decisiva':'ações decisivas'} para hoje.`:future.length?'Sua agenda está organizada. Antecipe a próxima conversa.':'Agenda livre para abrir novas oportunidades.';
  $('#todayCommandStrip').innerHTML=`<div class="today-command-summary"><span>O</span><div><small>AGENDA INTELIGENTE</small><h2>${escapeHtml(headline)}</h2><p>${linkedValue?`${formatMoney(linkedValue)} do pipeline estão conectados às próximas ações.`:'O Orbit organiza urgência, horário e impacto comercial em uma única leitura.'}</p></div></div><div class="today-command-numbers"><article class="${overdue.length?'attention':''}"><small>ATRASADAS</small><strong>${overdue.length}</strong></article><article><small>HOJE</small><strong>${dueToday.length}</strong></article><article><small>PRÓXIMAS</small><strong>${future.length}</strong></article></div><div class="today-agenda-preview">${agenda.length?agenda.map(item=>`<button type="button" data-today-agenda-client="${escapeHtml(item.client)}"><time>${item.date<today?'Atrasada':item.date===today?(item.time||'Hoje'):formatDate(item.date)}</time><span><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.client)} · ${escapeHtml(item.type)}</small></span><b>→</b></button>`).join(''):'<div><strong>Nenhum compromisso pendente</strong><small>Use este espaço para planejar novas conversas.</small></div>'}<button type="button" class="today-open-agenda" data-open-full-agenda>Ver agenda completa</button></div>`;
  document.querySelectorAll('[data-today-agenda-client]').forEach(button=>button.onclick=()=>{const client=getClients().find(item=>item.name===button.dataset.todayAgendaClient);if(client)openClientDrawer(client.id);else showView('activities')});
  $('[data-open-full-agenda]').onclick=()=>showView('activities');
}
function renderTodayActivities(){
  const activities=getActivities(),pending=activities.filter(activity=>!activity.done),deals=dealsVisibleToCurrentUser(getDeals()),ranked=rankTodayActivities(activities,deals,pipelineStages),stale=findStaleDeals(deals,getClients(),getStaleDealDays()),entries=[...ranked.map(item=>({type:'activity',score:item.score,item})),...stale.map(item=>({type:'stale',score:staleDealPriority(item),item}))].sort((a,b)=>b.score-a.score),today=todayISO();
  $('#todayPendingCount').textContent=pending.length+stale.length;$('#todayActivityHint').textContent=`${entries.length} ${entries.length===1?'prioridade':'prioridades'}`;
  $('#todayTasks').innerHTML=entries.length?entries.slice(0,6).map((entry,index)=>{if(entry.type==='stale'){const {deal,inactiveDays}=entry.item,stage=pipelineStages.find(item=>item.id===deal.stage)?.label||'Funil',health=healthForDeal(deal);return `<div class="task stale-deal-task"><button data-open-stale-deal="${deal.id}" title="Abrir negociação">↗</button><div><div class="task-title-line"><span class="task-intelligence-label attention">${index===0?'AGIR AGORA':'SEM MOVIMENTO'}</span><strong>Retomar ${escapeHtml(deal.title)}</strong></div><p>${escapeHtml(deal.client)} · ${escapeHtml(stage)}</p><small class="task-priority-reason">Saúde em ${health.label.toLowerCase()}: ${escapeHtml(health.reason)}. Essa negociação está esperando por você.</small></div><time>Há ${inactiveDays}d</time></div>`}const activity=entry.item.activity,label=activity.date<today?'ATRASADA':activity.date===today?'HOJE':'PRÓXIMA';return `<div class="task"><button class="complete-activity" data-complete-activity="${activity.id}">○</button><div><div class="task-title-line"><span class="task-intelligence-label ${activity.date<today?'attention':''}">${index===0?'PRIORIDADE · ':''}${label}</span><strong>${escapeHtml(activity.title)}</strong></div><p>${escapeHtml(activity.client)} · ${escapeHtml(activity.type)}</p><small class="task-priority-reason">${escapeHtml(todayPriorityReason(entry.item))}</small></div><time>${activity.time}</time></div>`}).join(''):'<div class="agenda-empty"><strong>Tudo em dia</strong>O Orbit não encontrou nenhuma pendência para hoje.</div>';
  renderTodayCommandCenter(activities,deals);bindActivityButtons();bindStaleDealButtons();renderTodayOrbitRecommendations();updateActivityBadge();updateDashboardMetrics();renderRoleFocus();applyTodayMetricSemantics();renderValueProof();
}
...
function renderProposals(){const search=$('#proposalSearch').value.toLowerCase(),all=getProposals(),visible=all.filter(item=>(appState.activeProposalFilter==='all'||item.status===appState.activeProposalFilter)&&(item.title+' '+item.client).toLowerCase().includes(search)),decision=all.filter(item=>['sent','viewed'].includes(item.status)),viewed=all.filter(item=>item.status==='viewed'),approved=all.filter(item=>item.status==='approved'),decided=approved.length+all.filter(item=>item.status==='refused').length,rate=decided?Math.round(approved.length/decided*100):0;$('#proposalsTotal').textContent=formatMoney(all.filter(item=>item.status!=='refused').reduce((sum,item)=>sum+Number(item.value),0));$('#proposalDecisionCount').textContent=decision.length;$('#proposalDecisionValue').textContent=formatMoney(decision.reduce((sum,item)=>sum+Number(item.value||0),0));$('#proposalViewedCount').textContent=viewed.length;$('#proposalApprovedRate').textContent=`${rate}%`;$('#proposalCommandTitle').textContent=viewed.length?`${viewed.length} ${viewed.length===1?'proposta foi visualizada':'propostas foram visualizadas'} e pede condução.`:decision.length?'Há propostas aguardando decisão.':'Transforme propostas em decisões.';$('#proposalCommandText').textContent=viewed.length?'O melhor momento para um retorno consultivo é agora.':decision.length?`${formatMoney(decision.reduce((sum,item)=>sum+Number(item.value||0),0))} estão em processo de decisão.`:'Crie uma oferta clara, acompanhe a interação e conduza o próximo passo.';$('#proposalPrimaryAction').textContent=viewed.length?'Revisar propostas visualizadas →':'Criar proposta premium →';$('#proposalPrimaryAction').onclick=()=>{if(viewed[0])openProposalPreview(viewed[0].id);else openProposalModal()};$('#proposalsList').innerHTML=visible.length?visible.map(item=>`<article class="record-card proposal-record"><button type="button" class="proposal-record-main" data-open-proposal="${escapeHtml(item.id)}"><span class="proposal-version">V${Number(item.version||1)}</span><div><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.client)} · Validade ${formatDate(item.validUntil)}</p><small>${item.items?.length||1} ${(item.items?.length||1)===1?'item':'itens'}${item.viewCount?` · ${item.viewCount} ${item.viewCount===1?'visualização':'visualizações'}`:''}</small></div></button><div class="record-cell"><small>VALOR</small><strong>${formatMoney(item.value)}</strong>${item.discount?`<span>${item.discount}% de desconto</span>`:''}</div><div><span class="record-status ${item.status}">${proposalStatusLabel(item.status)}</span>${item.acceptance?`<small class="proposal-accepted-by">Por ${escapeHtml(item.acceptance.name)}</small>`:''}</div><select class="record-action" data-proposal-status="${item.id}"><option value="">Alterar estado...</option><option value="draft">Rascunho</option><option value="sent">Enviada</option><option value="viewed">Visualizada</option><option value="approved">Aprovada</option><option value="refused">Recusada</option></select></article>`).join(''):orbitEmptyState('Uma boa proposta começa aqui','Crie uma oferta premium e conduza a oportunidade até a decisão.');document.querySelectorAll('[data-proposal-status]').forEach(select=>select.onchange=()=>changeProposalStatus(select.dataset.proposalStatus,select.value));document.querySelectorAll('[data-open-proposal]').forEach(button=>button.onclick=()=>openProposalPreview(button.dataset.openProposal))}
...
function ensureProposalCatalogPicker(){if($('#proposalCatalog'))return;$('#addProposalItem').insertAdjacentHTML('beforebegin','<span class="proposal-catalog-picker"><select id="proposalCatalog"><option value="">Selecionar do catálogo</option></select><button type="button" id="addCatalogToProposal">Adicionar</button></span>');$('#addCatalogToProposal').onclick=()=>{const item=getCatalog().find(record=>record.id===$('#proposalCatalog').value);if(!item)return;const empty=document.querySelector('.proposal-item-row');if(document.querySelectorAll('.proposal-item-row').length===1&&!empty.querySelector('[data-proposal-item="description"]').value)empty.remove();addProposalItem({description:item.name,quantity:1,unitPrice:Number(item.price||0)});$('#proposalCatalog').value='';$('#proposalCatalog').dispatchEvent(new Event('change'))}}
function openProposalModal(){const deals=getDeals();ensureProposalCatalogPicker();$('#proposalDeal').innerHTML='<option value="">Sem vínculo</option>'+deals.filter(deal=>deal.status!=='lost').map(deal=>`<option value="${deal.id}">${escapeHtml(deal.title)} · ${escapeHtml(deal.client)}</option>`).join('');$('#proposalCatalog').innerHTML='<option value="">Selecionar do catálogo</option>'+getCatalog().filter(item=>item.status==='active').map(item=>`<option value="${escapeHtml(item.id)}">${escapeHtml(item.name)}${item.price?` · ${formatMoney(item.price)}`:''}</option>`).join('');$('#proposalItemsEditor').innerHTML='';addProposalItem();$('#proposalModal').classList.remove('hidden');$('#proposalModal').setAttribute('aria-hidden','false');$('#proposalForm [name="validUntil"]').value=new Date(Date.now()+7*86400000).toISOString().slice(0,10);$('#proposalForm [name="title"]').focus();requestAnimationFrame(syncPremiumSelects)}
...
function ensureProposalPreview(){if($('#proposalPreviewDrawer'))return;document.body.insertAdjacentHTML('beforeend',`<div class="proposal-preview-backdrop hidden" id="proposalPreviewBackdrop"></div><aside class="proposal-preview-drawer hidden" id="proposalPreviewDrawer" aria-hidden="true"><header class="proposal-preview-toolbar"><div><small>NIVIONTECH CLOSE 2.0</small><strong>Proposta e decisão</strong></div><div><button type="button" id="compareProposalVersions">Comparar versões</button><button type="button" id="createProposalFollowUp">Criar follow-up</button><button type="button" class="room-action" id="createDealRoom">Criar sala segura</button><button type="button" id="createProposalVersion">Nova versão</button><button type="button" id="recordProposalView">Registrar visualização</button><button type="button" id="printProposal">Imprimir / PDF</button><button type="button" id="closeProposalPreview">×</button></div></header><div id="proposalPreviewDocument"></div></aside>`);$('#closeProposalPreview').onclick=closeProposalPreview;$('#proposalPreviewBackdrop').onclick=closeProposalPreview;$('#createProposalVersion').onclick=createProposalVersion;$('#recordProposalView').onclick=recordProposalView;$('#compareProposalVersions').onclick=showProposalComparison;$('#createProposalFollowUp').onclick=createProposalFollowUpAction;$('#createDealRoom').onclick=openDealRoomCreator;$('#printProposal').onclick=()=>{document.body.classList.add('printing-proposal');window.print();setTimeout(()=>document.body.classList.remove('printing-proposal'),300)}}
...
function createProposalVersion(){const proposals=getProposals(),original=proposals.find(item=>item.id===activeProposalPreviewId);if(!original)return;const copy=createProposalRevision(original);if(!original.familyId){original.familyId=original.id;saveProposals(proposals)}addEntityHistory(copy,'Nova versão criada',`Versão ${copy.version} criada a partir da versão ${Number(original.version||1)}`);proposals.push(copy);saveProposals(proposals);renderProposals();openProposalPreview(copy.id)}
function showProposalComparison(){const proposals=getProposals(),current=proposals.find(item=>item.id===activeProposalPreviewId),family=proposalFamily(proposals,current),previous=family.at(-2);if(!previous||previous.id===current.id){alert('Crie uma nova versão para comparar as alterações.');return}const changes=compareProposalVersions(previous,current),format=value=>typeof value==='number'?formatMoney(value):String(value||'Não informado');let panel=$('#proposalComparisonPanel');if(!panel){$('#proposalPreviewDocument').insertAdjacentHTML('afterbegin','<section id="proposalComparisonPanel" class="proposal-comparison"></section>');panel=$('#proposalComparisonPanel')}panel.innerHTML=`<header><div><small>EVOLUÇÃO DA PROPOSTA</small><h3>Versão ${previous.version||1} → versão ${current.version||1}</h3></div><button type="button" onclick="this.closest(\'.proposal-comparison\').remove()">×</button></header>${changes.length?changes.map(item=>`<article><strong>${escapeHtml(item.label)}</strong><del>${escapeHtml(format(item.before))}</del><span>→</span><ins>${escapeHtml(format(item.after))}</ins></article>`).join(''):'<p>Nenhuma alteração comercial identificada.</p>'}`}
function createProposalFollowUpAction(){const proposals=getProposals(),proposal=proposals.find(item=>item.id===activeProposalPreviewId);if(!proposal)return;const activity=proposalFollowUp(proposal),activities=getActivities(),duplicate=activities.some(item=>!item.done&&item.client===activity.client&&item.title===activity.title);if(!duplicate){activity.owner=appState.currentUser?.name||'';activities.push(activity);saveActivities(activities)}$('#createProposalFollowUp').textContent=duplicate?'Follow-up já existe ✓':'Follow-up criado ✓'}
function openDealRoomCreator(){const proposal=getProposals().find(item=>item.id===activeProposalPreviewId),deal=getDeals().find(item=>item.id===proposal?.dealId);if(!proposal||!deal){alert('Vincule a proposta a uma oportunidade para criar a sala segura.');return}let modal=$('#dealRoomModal');if(!modal){document.body.insertAdjacentHTML('beforeend',`<div id="dealRoomModal" class="modal hidden"><div class="modal-backdrop" data-close-room-modal></div><form class="modal-card deal-room-create"><header><div><p class="overline">SALA DA NEGOCIAÇÃO</p><h2>Compartilhar decisão com segurança</h2><small>Link individual, código de seis números e expiração automática.</small></div><button type="button" data-close-room-modal>×</button></header><div class="deal-room-security"><span>✓</span><div><strong>Dados mínimos e criptografados</strong><p>A sala recebe somente proposta, próximos passos e plano de decisão. O restante do CRM permanece privado.</p></div></div><div class="deal-form-grid"><label>Código de acesso<input name="accessCode" inputmode="numeric" maxlength="6" required></label><label>Validade<select name="expiresInDays"><option value="3">3 dias</option><option value="7" selected>7 dias</option><option value="15">15 dias</option><option value="30">30 dias</option></select></label></div><div id="dealRoomResult" class="deal-room-result hidden"></div><footer><button type="button" class="secondary" data-close-room-modal>Cancelar</button><button class="primary">Criar acesso seguro</button></footer></form></div>`);modal=$('#dealRoomModal');modal.querySelectorAll('[data-close-room-modal]').forEach(button=>button.onclick=()=>modal.classList.add('hidden'));modal.querySelector('form').onsubmit=createDealRoom}modal.dataset.proposalId=proposal.id;modal.querySelector('[name="accessCode"]').value=String(Math.floor(100000+Math.random()*900000));modal.querySelector('#dealRoomResult').classList.add('hidden');modal.classList.remove('hidden')}
async function createDealRoom(event){event.preventDefault();const modal=$('#dealRoomModal'),proposals=getProposals(),proposal=proposals.find(item=>item.id===modal.dataset.proposalId),deal=getDeals().find(item=>item.id===proposal?.dealId),plan=getClosingPlans().find(item=>item.dealId===deal?.id),form=event.currentTarget,accessCode=normalizeAccessCode(form.elements.accessCode.value),button=event.submitter;if(accessCode.length!==6){alert('Use um código com seis números.');return}button.disabled=true;button.textContent='Protegendo sala...';try{const response=await fetch('/api/deal-room',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({accessCode,expiresInDays:form.elements.expiresInDays.value,payload:dealRoomPayload({proposal,deal,plan,company:getCompany()})})}),data=await response.json();if(!response.ok)throw new Error(data.error);const url=new URL(data.url,location.origin).href;proposal.dealRoom={token:data.token,url,expiresAt:data.expiresAt,createdAt:new Date().toISOString(),accessCodeHint:`••••${accessCode.slice(-2)}`};addEntityHistory(proposal,'Sala segura criada',`Acesso válido até ${new Date(data.expiresAt).toLocaleDateString('pt-BR')}`);saveProposals(proposals);const result=$('#dealRoomResult');result.innerHTML=`<strong>Sala pronta para compartilhar</strong><label>Link<input value="${escapeHtml(url)}" readonly></label><p>Código: <b>${accessCode}</b> · envie separadamente ao cliente.</p><div><button type="button" data-copy-room>Copiar link</button><a href="${escapeHtml(url)}" target="_blank" rel="noopener">Visualizar sala</a></div>`;result.classList.remove('hidden');result.querySelector('[data-copy-room]').onclick=async()=>{await navigator.clipboard.writeText(url);result.querySelector('[data-copy-room]').textContent='Link copiado ✓'};button.textContent='Sala criada ✓'}catch{alert('Não foi possível criar a sala agora. Tente novamente.');button.disabled=false;button.textContent='Criar acesso seguro'}}
...
function exportBackup(){const backup={version:'1.1',product:'NivionTech CRM',exportedAt:new Date().toISOString(),company:getCompany(),owner:{...getOwner(),passwordHash:undefined,salt:undefined},catalog:getCatalog(),clients:getClients(),deals:getDeals(),activities:getActivities(),proposals:getProposals()};downloadFile(`niviontech-backup-${todayISO()}.json`,JSON.stringify(backup,null,2),'application/json')}
...
function convertProspectToClient(id){
  ...
  const client={id:'client-'+Date.now(),name:item.company,segment:getProspectProfile().segment,status:'Prospect',city:item.location||'Não informado',phone:'',email:'',website:item.website,owner:appState.currentUser?.name||'',source:'Orbit Prospectar',prospectScore:item.score,prospectEvidence:item.evidenceUrl,interactions:[{id:'interaction-'+Date.now(),title:'Empresa aprovada na prospecção do Orbit',text:item.evidenceSummary||'Origem pública registrada.',date:new Date().toISOString()}]};
...
function applyOrbitCommercialContinuation(dealId,choices){const deal=getDeals().find(item=>item.id===dealId);if(!deal)return;if(choices.includes('proposal')&&!getProposals().some(item=>item.dealId===deal.id)){const proposals=getProposals();proposals.unshift({id:`proposal-orbit-${Date.now()}`,dealId:deal.id,title:`Proposta · ${deal.title}`,client:deal.client,value:Number(deal.value||0),items:[{description:deal.title,quantity:1,unitPrice:Number(deal.value||0)}],subtotal:Number(deal.value||0),discount:0,discountValue:0,version:1,status:'draft',createdAt:new Date().toISOString(),createdBy:appState.currentUser?.name||'',viewCount:0,notes:deal.orbitMemory?.summary||''});saveProposals(proposals);addEntityHistory(deal,'Rascunho de proposta preparado pelo Orbit',formatMoney(deal.value))}if(choices.includes('plan'))ensureClosingPlanForDeal(deal);const deals=getDeals(),stored=deals.find(item=>item.id===deal.id);stored.history=deal.history;saveDeals(deals)}
...
function renderRoleFocus(){
  ...
  const byProfile={
    ...
    'Pós-venda':{icon:'○',title:'Foco no cliente',text:getClients().length+' clientes estão disponíveis para acompanhamento e expansão.',target:'success',action:'Acompanhar carteira'},
...
function renderReports(){
  ...
  const executiveTitle=management.riskValue?`${formatMoney(management.riskValue)} da receita precisam de proteção.`:open.length?`Seu funil tem ${open.length} ${open.length===1?'oportunidade ativa':'oportunidades ativas'}.`:won.length?`${won.length} ${won.length===1?'venda concluída':'vendas concluídas'} sustentam o resultado.`:'Sua operação está pronta para a primeira oportunidade.';
  const executiveAction=management.recommendations[0]||(!open.length?'Cadastre uma oportunidade e defina o próximo passo.':'Mantenha as próximas ações e evidências atualizadas.');
  setText('#executiveStoryTitle',executiveTitle);setText('#executiveStoryText',`Previsão por evidência: ${formatMoney(management.weighted)} · Conversão atual: ${conversion}%.`);setText('#executiveStoryAction',executiveAction);const executiveButton=$('#executiveStoryButton');if(executiveButton){executiveButton.textContent=management.riskValue?'Proteger receita →':open.length?'Revisar pipeline →':'Criar oportunidade →';executiveButton.onclick=()=>{showView('pipeline');if(!open.length)openDealModal()}}
...
function openOrbitAttention(){
  ...
  document.body.classList.add('attention-open','orbit-cinematic');
}

function closeOrbitAttention(){
  $('#orbitAttentionPanel')?.classList.remove('open');
  $('#orbitAttentionBackdrop')?.classList.remove('open');
  document.body.classList.remove('attention-open','orbit-cinematic');
}
...
function createFullLocalBackup(reason='manual'){
  const storage=collectSyncStorage(localStorage);
...
async function restoreFullLocalBackup(file){
  ...
  const entries=backup?.storage&&typeof backup.storage==='object'?Object.entries(backup.storage):[];
  const valid=backup&&backup.app==='NivionTech CRM'&&backup.format==='niviontech-full-backup'&&entries.length>0&&entries.length<=120&&entries.every(([key,value])=>key.startsWith('niviontech_')&&typeof value==='string'&&!['niviontech_session','niviontech_owner_password','niviontech_password_hash'].includes(key))&&file.size<=2_500_000;
  ...
  [...Array(localStorage.length)].map((_,index)=>localStorage.key(index)).filter(key=>key?.startsWith('niviontech_')&&!['niviontech_sync_meta','niviontech_device_id'].includes(key)).forEach(key=>localStorage.removeItem(key));
