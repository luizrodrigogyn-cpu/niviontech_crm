...
function renderTodayActivities(){
  ...
  $('#todayTasks').innerHTML=entries.length?entries.slice(0,6).map((entry,index)=>{if(entry.type==='stale'){const {deal,inactiveDays}=entry.item,stage=pipelineStages.find(item=>item.id===deal.stage)?.label||'Funil',health=healthForDeal(deal);return `<div class="task stale-deal-task"><button data-open-stale-deal="${deal.id}" title="Abrir negociação">↗</button><div><span class="task-intelligence-label attention">${index===0?'AGIR AGORA':'SEM MOVIMENTO'}</span><strong>Retomar ${escapeHtml(deal.title)}</strong><p>${escapeHtml(deal.client)} · ${escapeHtml(stage)}</p><small class="task-priority-reason">Saúde em ${health.label.toLowerCase()}: ${escapeHtml(health.reason)}. Essa negociação está esperando por você.</small></div><time>Há ${inactiveDays}d</time></div>`}const activity=entry.item.activity,label=activity.date<today?'ATRASADA':activity.date===today?'HOJE':'PRÓXIMA';return `<div class="task"><button class="complete-activity" data-complete-activity="${activity.id}">○</button><div><span class="task-intelligence-label ${activity.date<today?'attention':''}">${index===0?'PRIORIDADE · ':''}${label}</span><strong>${escapeHtml(activity.title)}</strong><p>${escapeHtml(activity.client)} · ${escapeHtml(activity.type)}</p><small class="task-priority-reason">${escapeHtml(todayPriorityReason(entry.item))}</small></div><time>${activity.time}</time></div>`}).join(''):'<div class="agenda-empty"><strong>Tudo em dia</strong>O Orbit não encontrou nenhuma pendência para hoje.</div>';
